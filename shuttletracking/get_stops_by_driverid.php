...
class stopService {
    ...
    function getStopsInfo($driver_id){
        GLOBAL $con;
        $stops = array();
        $sqlDriverQuery = "SELECT * FROM `tbl_shuttle_parkinglot_drivers` where SPD_DVR_DriverID = '".$driver_id."' and SPD_IsActive = '1' group by SPD_DLG_DriverTripLogID";
        ...
        if ($countDriverResult > 0) {
            ...
            while ($results = mysqli_fetch_assoc($sqlDriverResult)) {
                    $sqlDriverSubQuery = "SELECT GROUP_CONCAT(tspd.SPD_ParkingLotID) as SPD_ParkingLotIDs, tsts.*, tsdt.DLG_TripName, tsdt.DLG_Phone, tspd.SPD_DVR_DriverID FROM `tbl_shuttle_parkinglot_drivers` tspd "
                        ...
                        . " WHERE tspd.SPD_DVR_DriverID = '".$driver_id."' and tspd.SPD_DLG_DriverTripLogID = '".$results['SPD_DLG_DriverTripLogID']."' and tspd.SPD_IsActive = '1' Group by tsts.DTP_TripStopID";
