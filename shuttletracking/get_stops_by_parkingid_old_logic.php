    ...
    function getStopsInfo($parking_id){
        ...
        if ($countDriverResult > 0) {
            while ($results = mysqli_fetch_assoc($sqlDriverResult)) {
                    $sqlDriverSubQuery = "SELECT tsts.*, tsdt.DLG_TripName, tspd.SPD_DVR_DriverID FROM `tbl_shuttle_parkinglot_drivers` tspd "
                    ...
                    if ($countDriverSubResult > 0) {
                        ...
                        while ($subResults = mysqli_fetch_assoc($sqlDriverSubResult)) {
							$subResults['totalStopsCount'] = (string)$countDriverSubResult;
                            $stops_new[] = $subResults;
                        }
                        if(!empty($stops_new)){
                            $stops = array_merge($stops,$this->setPreviousNextStopIDs($stops_new));
                        }
                    }            
            }
        }
