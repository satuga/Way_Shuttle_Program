...
class Dine
{
	...
	{
		$sql="SELECT id,Main_Menu,Start_Time,End_Time FROM menus WHERE menuStatus='Active' AND merchantID=".mysql_real_escape_string($_REQUEST['id'])." GROUP BY Main_Menu LIMIT 3";
   ...
   function GetCoupons($rest)
    ...
    function Restaurant_OpenHours_allday($id,$orderfor)
	{
		...
		//echo $sql;
		$exe=mysql_query($sql);
		while($CC=mysql_fetch_array($exe)){
		if($CC['ST']==$CC['ET']) {
			$ST="";$ET=""; } 
		else {$ST=$CC['ST'];$ET=$CC['ET']; }
		if($CC['STO']==$CC['ETO']) {
...
$message .=	"Content-Type: image/$ext; name=\"$google_icon_file.$ext\"\r\n"
	...
	function AddHelpfull()
	{
		...
		mysql_query($sql);
		$sql="INSERT INTO tbl_reviewhelpful(Help_Review,Help_Restaurant,Help_UserID,Help_Created)VALUES(".mysql_real_escape_string($_REQUEST['Review_ID']).",'".mysql_real_escape_string($_REQUEST['ID'])."','".$User_ID."',Now())";
		mysql_query($sql);
	}
	function AddLike()
	{
