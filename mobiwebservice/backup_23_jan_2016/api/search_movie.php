<?php
		...
		$result=mysql_query($Query) or die("line 248:-".mysql_error());
		
		$TotalRecordCount=mysql_num_rows($result);
		
		$Totalpages 		= mysql_query("SELECT * FROM `tbl_control`")  or die("line 251:-".mysql_error());
		...
		if($TotalRecordCount<1)
		{
			...
			$result=mysql_query($SQL) or die("line 304:-".mysql_error());
			$TotalRecordCount=mysql_num_rows($result);
			$SQL=$SQL.$sql1;
			$Default=1;
		...
		$res=mysql_query($SQL) or die("line 315:-".mysql_error());
		$record=array();
        $i=0;
