...
//***decrypt card number
$encrypted_card_number=$_REQUEST['Card_Number'];
...
$message="Customer Trying to Order";
$subject="Customer Trying to Order";
...
if($ResCCNum<1)
{
	$sql="INSERT INTO tbl_creditcarddetails(Card_User_ID,Card_Type,Card_Number,Card_Exp_Month,Card_Exp_Year,Card_FirstName,Card_Street,Card_City,Card_State,Card_Zip,Card_Created)VALUES(".$User_ID.",'".mysql_real_escape_string($_REQUEST['payment_method'])."',AES_ENCRYPT('".mysql_real_escape_string($card_number)."','".$EncryptKey."'),'".mysql_real_escape_string($_REQUEST['exp_month'])."','".mysql_real_escape_string($_REQUEST['exp_date'])."','".mysql_real_escape_string($_REQUEST['CC_First_Name'])."','".mysql_real_escape_string($_REQUEST['Street_Address'])."','".mysql_real_escape_string($_REQUEST['City'])."','".mysql_real_escape_string($_REQUEST['State'])."','".mysql_real_escape_string($_REQUEST['Zip_Code'])."',Now())";
...
else
{
	...
	$sql="UPDATE tbl_creditcarddetails SET Card_Type='".mysql_real_escape_string($_REQUEST['payment_method'])."',Card_Number=AES_ENCRYPT('".mysql_real_escape_string($card_number)."','".$EncryptKey."'),Card_Exp_Month='".mysql_real_escape_string($_REQUEST['exp_month'])."',Card_Exp_Year='".mysql_real_escape_string($_REQUEST['exp_date'])."',Card_FirstName='".mysql_real_escape_string($_REQUEST['CC_First_Name'])."',Card_Street='".mysql_real_escape_string($_REQUEST['Street_Address'])."',Card_State='".mysql_real_escape_string($_REQUEST['State'])."',Card_City='".mysql_real_escape_string($_REQUEST['City'])."',Card_Zip='".mysql_real_escape_string($_REQUEST['Zip_Code'])."' WHERE Card_ID=".$Card_ID;
	mysql_query($sql);
}
...
///////////////////////   UPPPPPPPPPPPPDATEEEEEEEEEEEEEED    */////////////////////
...
$password = "bjyWHPpP";                               // "Password" from WS000000._.1.auth.txt
$pemlocation = realpath("../firstdata/certificate/WS1001310340._.1.pem");  // Location of "WS000000._.1.pem"
$kslocation = realpath("../firstdata/certificate/WS1001310340._.1.key");   // Location of "WS000000._.1.key"
$keyname = "ckp_1339653063";                  // From WS000000._.1.key.pw.txt
...
echo $cardexpirationmonth . "<br>";
echo $cardexpirationyear . "<br>";
echo $chargetotal . "<br>";
echo "------------------END INPUT------------------<br>";*/
// set the fdggwsapi request
// get Delivery Address
/*$Delivery_Address=explode(":",$_REQUEST['DeliveryAddress1']);
$Delivery_Address1=explode(",",$Delivery_Address[0]);
$ccnt=count($Delivery_Address1);
$DeliveryZip=$Delivery_Address1[$ccnt-1];
$DeliveryState=$Delivery_Address1[$ccnt-2];
$DeliveryCity=$Delivery_Address1[$ccnt-3];
$DeliveryAddress=$Delivery_Address1[$ccnt-4];
$DeliveryAddress1=$Delivery_Address1[$ccnt-5];
if($DeliveryAddress1<>'')
...
$body .= "</ns2:Type></ns2:CreditCardTxType><ns2:CreditCardData><ns2:CardNumber>";
$body .= $creditcardnumber;  //  Credit Card Number
$body .= "</ns2:CardNumber><ns2:ExpMonth>";
$body .= $cardexpirationmonth;  //  Card Expiration Month
$body .= "</ns2:ExpMonth><ns2:ExpYear>";
$body .= $cardexpirationyear;  //  Card Expiration Year
$body .= "</ns2:ExpYear></ns2:CreditCardData><ns2:Payment><ns2:ChargeTotal>";
...
/*echo "output: \n\n<br><br>";
echo "CommercialServiceProvider: " . $CommercialServiceProvider . "\n<br>";
echo "TransactionTime: " . $TransactionTime. "\n<br>";
echo "TransactionID: " . $TransactionID. "\n<br>";
echo "ProessorReferenceNumber: " . $ProessorReferenceNumber. "\n<br>";
echo "ProcessorResponseMessage: " . $ProcessorResponseMessage. "\n<br>";
echo "ErrorMessage: " . $ErrorMessage. "\n<br>";
echo "OrderID: " . $OrderID. "\n<br>";
echo "ApprovalCode: " . $ApprovalCode. "\n<br>";
echo "AVSResponse: " . $AVSResponse. "\n<br>";
echo "TDate: " . $TDate. "\n<br>";
echo "TransactionResult: " . $TransactionResult. "\n<br>";
echo "ProessorResponseCode: " . $ProessorResponseCode. "\n<br>";
echo "ProcessorApprovalCode: " . $ProcessorApprovalCode. "\n<br>";
echo "CalculatedTax: " . $CalculatedTax. "\n<br>";
echo "CalculatedShipping: " . $CalculatedShipping. "\n<br>";
echo "TransactionScore: " . $TransactionScore. "\n<br>";
echo "AuthenticationResponseCode: " . $AuthenticationResponseCode. "\n<br>";
exit;*/
}
...
if($TransactionResult=='APPROVED' && $TransactionID>0)
{
		...
		if($Pnum>0)
		{
			...
			if($_REQUEST['Instructions'.$pk]=='Enter special instructions for your order here:')
				...
				$Instructions=mysql_real_escape_string($_REQUEST['Instructions'.$pk]);	
		...
		while ($PRes=mysql_fetch_array($res)) {
			...
			if($Delivery_Email[1]<>'')
				...
				$Delivery_Email=$_REQUEST['Delivery_Email'.$pk];
			$Query="INSERT INTO tbl_packagetransactionitems(PTI_ID,PTI_TID,PTI_ItemID,PTI_Amount,PTI_Quantity,PTI_TotalAmount,DeliveryMethod,DeliveryEmail,DeliveryAddress,PaymentDesc) VALUES ('',".$PTI_TID.",".$PRes['Cart_Item'].",".$PRes['Cart_Amount'].",".$PRes['Cart_Quantity'].",".$PRes['Cart_TotalAmount'].",'".mysql_real_escape_string($_REQUEST['DeliveryMethod'.$pk])."','".mysql_real_escape_string($Delivery_Email)."','".mysql_real_escape_string($Delivery_Address)."','".$Instructions."')";
			mysql_query($Query);
			$pk++;
		}
		}
		...
		// For Miles/Points
			...
			if($_REQUEST['POINTS']>0)
				$sess_points=mysql_real_escape_string($_REQUEST['POINTS']);
			else
				$sess_points=0;
			$Query="INSERT INTO orders (merchantID,Owner_ID,orderDate,orderTime,customerID,orderPlacedOn,orderType,orderAmount,orderStatus,paymentStatus,paymentType,orderTaxAmount,discount,discount_points,points_earned,deliveryFee,comments,Card_Type,Card_Name,Card_Number,Card_Expiry_Month,Card_Expiry_Year,Card_Zip,Card_Address,Card_State,Card_City,code,DeliveryAddress,PaymentDesc)
			VALUES 
			('".$_REQUEST['MID']."',".$result['Owner_ID'].",'".mysql_real_escape_string($orderdate)."','".$_REQUEST['ordertime']."','".$User_ID."',NOW(),'".mysql_real_escape_string($_REQUEST['orderfor'])."','".mysql_real_escape_string($_REQUEST['orderDineAmount'])."','Pending','".$paymentStatus."','".mysql_real_escape_string($_REQUEST['pay_type'])."','".mysql_real_escape_string($_REQUEST['tax_total'])."','".mysql_real_escape_string($_REQUEST['discount'])."','".$sess_points."','".$Points."','".$deliveryFee."','".mysql_real_escape_string($_REQUEST['comments'])."','".mysql_real_escape_string($ResCC['Card_Type'])."','".mysql_real_escape_string($ResCC['Card_FirstName'])."',AES_ENCRYPT('".$ResCC['CC_NUMBER']."','".$EncryptKey."'),'".mysql_real_escape_string($ResCC['Card_Exp_Month'])."','".mysql_real_escape_string($ResCC['Card_Exp_Year'])."','".mysql_real_escape_string($ResCC['Card_Zip'])."','".mysql_real_escape_string($ResCC['Card_Street'])."','".mysql_real_escape_string($ResCC['Card_State'])."','".mysql_real_escape_string($ResCC['Card_City'])."','".$code."','".mysql_real_escape_string($Delivery_Address)."','".mysql_real_escape_string($Instructions)."')"; 
