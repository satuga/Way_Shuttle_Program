...
if($device_id != '' && $user_id != '')
{
	...
	if(count($records) > 0)
	{
		foreach($records as $k=>$v)	
		{
			...
			$get_loc = @mysql_fetch_assoc(mysql_query("select tbl_parkinglocations.Park_Address AS Title,tbl_parkinglocations.Park_Name from tbl_parking INNER JOIN tbl_parkinglocations ON tbl_parkinglocations.Park_ID=tbl_parking.P_Location where P_ID=".$v['Cart_ServiceID']));
		...
		$total = @mysql_fetch_array(mysql_query("select SUM(TotalAmount) AS TOTAL from tbl_cart where Sess_ID='".$device_id."' and Cart_UserID ='".$user_id."'"));
