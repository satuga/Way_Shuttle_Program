...
$bexe=mysql_query($bsql);
$bnum=mysql_num_rows($bexe);
$bres=mysql_fetch_array($bexe);
...
$rnum=mysql_num_rows($rexe);
		if($rnum>0)
		{
			$ki=1;
			while($rres=mysql_fetch_array($rexe))
			{
				$asql="SELECT AVG(txt".$ki.") AS RATING FROM tbl_reviews WHERE R_Type='Dine' AND list_id =".$_REQUEST['id'];
				$aexe=mysql_query($asql);
				$anum=mysql_num_rows($aexe);
				$ares=mysql_fetch_array($aexe);
