...
if($driver_id != '')
{
	...
	$driver_order = "select m.merchantName as res_name,m.telephone as res_telephone,m.geoLat as res_lat,m.geoLong as res_long,m.contactAddress as res_address,m.logo as res_logo,reg.firstname as user_firstname,reg.lastname as user_lastname,reg.home_phone as user_homephone,reg.work_phone as user_workphone,reg.mobile_phone as user_mobilephone,o.*,ride.status as order_status from driver_accept_ride as ride 
	...
	$driver_query = mysql_query($driver_order);
	$rows = mysql_num_rows($driver_query);
	if($rows > 0)
	{
		$i=0;
		while($rowss = mysql_fetch_assoc($driver_query))
		{
			$all_orders = array();
			$order = "select it.*,items.* from order_items as items left join items as it on it.id = items.itemID where items.orderID = '".$rowss['id']."'";
			$order_detail = mysql_query($order);
			while($order_info = mysql_fetch_assoc($order_detail))
			{
