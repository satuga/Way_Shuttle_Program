...
if($latitude != '' && $longitude != '')
{
    ...
    * cos((`delivery_lat`*pi()/180)) * cos((('$longitude'- `delivery_long`)*pi()/180))))*180/pi())*60*1.1515) 
	...
	while($aRow=mysql_fetch_assoc($res))
	{
		...
		$res_detail_info = mysql_query($res_detail);
		
		$user_detail = "select firstname,lastname,home_phone,work_phone,mobile_phone from tbl_registeration where id = '".$aRow['customerID']."'";
		$user_detail_info = mysql_query($user_detail);
		$dd = array();
		while($user_info = mysql_fetch_assoc($user_detail_info))
		{
			$dd[] = $user_info;
		}
		...
		while($res_info=mysql_fetch_assoc($res_detail_info))
		{
			$info[$i]['id'] = $aRow['id'];
