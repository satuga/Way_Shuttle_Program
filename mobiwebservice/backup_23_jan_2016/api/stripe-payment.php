...
if ($_REQUEST["User_ID"])
{
	...
	$address_qu = mysql_query($del_query);
	$address_count = mysql_num_rows($address_qu);
	if($address_count > 0)
	{
		while($d_address=mysql_fetch_array($address_qu))
		{
	...
	else
	{
		...
		$sql="UPDATE tbl_creditcarddetails SET Card_Type='".mysql_real_escape_string($_REQUEST['payment_method'])."',Card_Number='".$encrypted_card_number."',Card_Exp_Month='".mysql_real_escape_string($_REQUEST['exp_month'])."',Card_Exp_Year='".mysql_real_escape_string($_REQUEST['exp_date'])."',Card_FirstName='".mysql_real_escape_string($_REQUEST['CC_First_Name'])."',Card_Street='".mysql_real_escape_string($_REQUEST['Street_Address'])."',Card_State='".mysql_real_escape_string($_REQUEST['State'])."',Card_City='".mysql_real_escape_string($_REQUEST['City'])."',Card_Zip='".mysql_real_escape_string($_REQUEST['Zip_Code'])."' WHERE Card_ID=".$Card_ID;
		mysql_query($sql);
	}
	...
	if($_REQUEST['pay_type']=='CreditCard')
	{
		 ...
		 //$amount  = round($_REQUEST['orderAmount']);
	...
	if($charge->paid == "1" && $charge->id != "")
	{
		...
		if($DineTran>0)
		{
			...
			VALUES
			('".$_REQUEST['MID']."',".$result['Owner_ID'].",'".mysql_real_escape_string($orderdate)."','".$_REQUEST['ordertime']."','".$User_ID."',NOW(),'".mysql_real_escape_string($_REQUEST['orderfor'])."','".mysql_real_escape_string($_REQUEST['orderAmount'])."','".$tip."','Confirmed','".$paymentStatus."','".mysql_real_escape_string($_REQUEST['pay_type'])."','".mysql_real_escape_string($_REQUEST['tax_total'])."','".mysql_real_escape_string($_REQUEST['discount'])."','".$sess_points."','".$Points."','".$deliveryFee."','".mysql_real_escape_string($_REQUEST['comments'])."','".mysql_real_escape_string($ResCC['Card_Type'])."','".mysql_real_escape_string($ResCC['Card_FirstName'])."',AES_ENCRYPT('".$ResCC['CC_NUMBER']."','".$EncryptKey."'),'".mysql_real_escape_string($ResCC['Card_Exp_Month'])."','".mysql_real_escape_string($ResCC['Card_Exp_Year'])."','".mysql_real_escape_string($ResCC['Card_Zip'])."','".mysql_real_escape_string($ResCC['Card_Street'])."','".mysql_real_escape_string($ResCC['Card_State'])."','".mysql_real_escape_string($ResCC['Card_City'])."','".$code."','".mysql_real_escape_string($Delivery_Address)."','".mysql_real_escape_string($Instructions)."','".$_REQUEST['delivery_lat']."','".$_REQUEST['delivery_long']."')";
