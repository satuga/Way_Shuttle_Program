...
class Dine
{
	...
	/*
	Develoepr Name :Hitesh Tank
  Function Name: MergeShowCart
	Date : 5,Feb 2016
	*/

	function MergeShowCart(){
		GLOBAL $con;
		if(sanitize($_REQUEST['user_id']) !=""){
		 $Query="SELECT tbl_cart.*,items.itemName,items.Cus_Pizza from tbl_cart LEFT JOIN items ON items.id=tbl_cart.Cart_ServiceID where Cart_UserID='".sanitize($_REQUEST['user_id'])."'";
		}else{
			$Query="SELECT tbl_cart.*,items.itemName,items.Cus_Pizza from tbl_cart LEFT JOIN items ON items.id=tbl_cart.Cart_ServiceID where Sess_ID='".sanitize($_REQUEST['device_id'])."'";
		}
        ...
        function MergeShowCartPayLater(){
		GLOBAL $con;
		if(sanitize($_REQUEST['User_ID']) !=""){
		 $Query="SELECT tbl_cart.*,items.itemName,items.Cus_Pizza from tbl_cart LEFT JOIN items ON items.id=tbl_cart.Cart_ServiceID where Cart_UserID='".sanitize($_REQUEST['User_ID'])."'";
		}else{
	...
	function view_Restaurants() {
			...
			if(sanitize($_REQUEST['Keyword'])<>'')
			$sql.=" AND (merchantName LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%' OR merchantName LIKE '%".mysqli_real_escape_string(strtoupper(sanitize($_REQUEST['Keyword'])))."%' OR merchantName LIKE '%".mysqli_real_escape_string(strtolower(sanitize($_REQUEST['Keyword'])))."%' OR merchantName LIKE '%".mysqli_real_escape_string(ucwords(sanitize($_REQUEST['Keyword'])))."%' OR merchantName LIKE '%".mysqli_real_escape_string(ucfirst(sanitize($_REQUEST['Keyword'])))."%' OR contactName LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%' OR contactAddress LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%' OR crossStreet LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%')";
	...
	function Add_Restaurant()
	{
		GLOBAL $Map_API;
		GLOBAL $con;
		$State=mysqli_real_escape_string($con, sanitize($_REQUEST['State']));
		$City=mysqli_real_escape_string($con, sanitize($_REQUEST['City']));
		$cuisines=implode(",",sanitize($_REQUEST['cuisine']));
		$loc=$City.",".$State;
		$where = stripslashes(sanitize($_REQUEST['Zip_Code']));
		...
		if(sanitize($_REQUEST['ID'])>0)
		{
			$sql="UPDATE merchant SET merchantName='".mysqli_real_escape_string($con, sanitize($_REQUEST['merchantname']))."',contactName='".mysqli_real_escape_string($con, sanitize($_REQUEST['contactname']))."',cuisine='".$cuisines."',state='".trim($State)."',city='".$City."',contactAddress='".mysqli_real_escape_string($con, sanitize($_REQUEST['streetaddress']))."',crossStreet='".mysqli_real_escape_string($con, sanitize($_REQUEST['crossstreets']))."',postalCode='".mysqli_real_escape_string($con, sanitize($_REQUEST['Zip_Code']))."',telephone='".mysqli_real_escape_string($con, sanitize($_REQUEST['phonenumber']))."',faxNumber='".mysqli_real_escape_string($con, sanitize($_REQUEST['faxnumber']))."',gsmNumber='".mysqli_real_escape_string($con, sanitize($_REQUEST['gsmnumber']))."',email='".mysqli_real_escape_string($con, sanitize($_REQUEST['email']))."',delivery='".mysqli_real_escape_string($con, sanitize($_REQUEST['deliveryavailable']))."',takeout='".mysqli_real_escape_string($con, sanitize($_REQUEST['takeoutavailable']))."',tablebooking='".mysqli_real_escape_string($con, sanitize($_REQUEST['tablebooking']))."',phoneorderonly='".mysqli_real_escape_string($con, sanitize($_REQUEST['phoneorderonly']))."',payment_cash='".mysqli_real_escape_string($con, sanitize($_REQUEST['payment_cash']))."',payment_cc='".mysqli_real_escape_string($con, sanitize($_REQUEST['payment_cc']))."',Neighborhood='".mysqli_real_escape_string($con, sanitize($_REQUEST['Neighborhood']))."',executive_chef='".mysqli_real_escape_string($con, sanitize($_REQUEST['executive_chef']))."',dress_code='".mysqli_real_escape_string($con, sanitize(sanitize($_REQUEST['dress_code']))."',additional_service='".mysqli_real_escape_string($con, $_REQUEST['additional_service']))."',public_transit='".mysqli_real_escape_string($con, sanitize($_REQUEST['public_transit']))."',parking='".mysqli_real_escape_string($con, sanitize($_REQUEST['parking']))."',private_party_facility='".mysqli_real_escape_string($con, sanitize($_REQUEST['private_party_facility']))."',private_party_contact='".mysqli_real_escape_string($con, sanitize($_REQUEST['private_party_contact']))."',catering='".mysqli_real_escape_string($con, sanitize($_REQUEST['catering']))."',aboutUs='".mysqli_real_escape_string($con, sanitize($_REQUEST['aboutus']))."',dining_style='".mysqli_real_escape_string($con, sanitize($_REQUEST['dining_style']))."',promotions='".mysqli_real_escape_string($con, sanitize($_REQUEST['promotions']))."',entertainment='".mysqli_real_escape_string($con, sanitize($_REQUEST['entertainment']))."',timezone='".mysqli_real_escape_string($con, sanitize($_REQUEST['timezone']))."',geoLong='".$long."',geoLat='".$lat."' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
			$rep=2;
			$rec=mysqli_query($con, $sql);
			$Merchant_ID=sanitize($_REQUEST['ID']);
		}
		else
		{
			$sql="INSERT INTO merchant(Res_UserID,merchantName,contactName,state,sanitize(city,contactAddress,cuisine,crossStreet,postalCode,telephone,faxNumber,gsmNumber,email,status,delivery,takeout,tablebooking,phoneorderonly,payment_cash,payment_cc,createdOn,Neighborhood,executive_chef,dress_code,additional_service,public_transit,parking,private_party_facility,private_party_contact,catering,aboutUs,dining_style,promotions,entertainment,timezone,geoLong,geoLat) VALUES('".$_SESSION['User_ID']."','".mysqli_real_escape_string($con, sanitize($_REQUEST['merchantname']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['contactname']))."','".trim($State)."','".$City."','".mysqli_real_escape_string($con, sanitize($_REQUEST['streetaddress']))."','".$cuisines."','".mysqli_real_escape_string($con, sanitize($_REQUEST['crossstreets']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['Zip_Code']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['phonenumber']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['faxnumber']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['gsmnumber']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['email']))."','1','".mysqli_real_escape_string($con, sanitize($_REQUEST['deliveryavailable']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['takeoutavailable']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['tablebooking']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['phoneorderonly']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['payment_cash']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['payment_cc']))."',Now(),'".mysqli_real_escape_string($con, sanitize($_REQUEST['Neighborhood']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['executive_chef']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['dress_code']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['additional_service']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['public_transit']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['parking']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['private_party_facility']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['private_party_contact']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['catering']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['aboutus']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['dining_style']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['promotions']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['entertainment']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['timezone']))."','".$long."','".$lat."')";
			$rep=1;
			$rec=mysqli_query($con, $sql);
	...
	else
	{
		...
		$new_file_name=sanitize($_REQUEST['logo']);
	}
		if(sanitize($_REQUEST['popular'])=='')
			$popular=0;
		else
			$popular=sanitize($_REQUEST['popular']);
		if($_REQUEST['spicy']=='')
			$spicy=0;
		else
			$spicy=sanitize($_REQUEST['spicy']);
		if($_REQUEST['veggie']=='')
			$veggie=0;
		else
			$veggie=sanitize($_REQUEST['veggie']);
		if($_REQUEST['chef_special']=='')
			$chef_special=0;
		else
			$chef_special=sanitize($_REQUEST['chef_special']);

		if($_REQUEST['price1']=='')
			$price1=0;
		else
			$price1=sanitize($_REQUEST['price1']);
		if($_REQUEST['price2']=='')
			$price2=0;
		else
			$price2=sanitize($_REQUEST['price2']);
		if($_REQUEST['price3']=='')
			$price3=0;
		else
			$price3=sanitize($_REQUEST['price3']);
		if($_REQUEST['price4']=='')
			$price4=0;
		else
			$price4=sanitize($_REQUEST['price4']);
		if($_REQUEST['price5']=='')
			$price5=0;
		else
			$price5=sanitize($_REQUEST['price5']);
		if($_REQUEST['price6']=='')
			$price6=0;
		else
			$price6=sanitize($_REQUEST['price6']);
		if($_REQUEST['price7']=='')
			$price7=0;
		else
			$price7=sanitize($_REQUEST['price7']);
		if($_REQUEST['price8']=='')
			$price8=0;
		else
			$price8=sanitize($_REQUEST['price8']);
		if($_REQUEST['price9']=='')
			$price9=0;
		else
			$price9=sanitize($_REQUEST['price9']);
		if($_REQUEST['price10']=='')
			$price10=0;
		else
			$price10=sanitize($_REQUEST['price10']);

		if($_REQUEST['price11']=='')
			$price11=0;
		else
			$price11=sanitize($_REQUEST['price11']);
		if($_REQUEST['price12']=='')
			$price12=0;
		else
			$price12=sanitize($_REQUEST['price12']);
		if($_REQUEST['price13']=='')
			$price13=0;
		else
			$price13=sanitize($_REQUEST['price13']);
		if($_REQUEST['price14']=='')
			$price14=0;
		else
			$price14=sanitize($_REQUEST['price14']);
		if($_REQUEST['price15']=='')
			$price15=0;
		else
			$price15=sanitize($_REQUEST['price15']);
		if($_REQUEST['price16']=='')
			$price16=0;
		else
			$price16=sanitize($_REQUEST['price16']);
		if($_REQUEST['price17']=='')
			$price17=0;
		else
			$price17=sanitize($_REQUEST['price17']);
		if($_REQUEST['price18']=='')
			$price18=0;
		else
			$price18=sanitize($_REQUEST['price18']);
		if($_REQUEST['price19']=='')
			$price19=0;
		else
			$price19=sanitize($_REQUEST['price19']);
		if($_REQUEST['price20']=='')
			$price20=0;
		else
			$price20=sanitize($_REQUEST['price20']);

		if($_REQUEST['Cus_Pizza']=='')
			$Cus_Pizza=0;
		else
			$Cus_Pizza=sanitize($_REQUEST['Cus_Pizza']);
		if($_REQUEST['Multiple_Toppings']=='')
			$Multiple_Toppings=0;
		else
			$Multiple_Toppings=sanitize($_REQUEST['Multiple_Toppings']);
		if($_REQUEST['Free_Toppings']=='')
			$Free_Toppings=0;
		else
			$Free_Toppings=sanitize($_REQUEST['Free_Toppings']);
		if($_REQUEST['Max_Toppings']=='')
			$Max_Toppings=0;
		else
			$Max_Toppings=sanitize($_REQUEST['Max_Toppings']);

		if($_REQUEST['Item_ID']>0)
		{
			$sql="UPDATE items SET itemName='".mysqli_real_escape_string($con, sanitize($_REQUEST['itemname']))."',itemImage='".$new_file_name."',
			itemDescription='".mysqli_real_escape_string($con, sanitize($_REQUEST['description']))."',
			itemStatus='".sanitize($_REQUEST['status'])."',popular='".$popular."',spicy='".$spicy."',
			veggie='".$veggie."',chef_special='".$chef_special."',itemPrice='".mysqli_real_escape_string($price1)."',
			itemPrice1='".mysqli_real_escape_string($price2)."',itemPrice2='".mysqli_real_escape_string($price3)."',
			itemPrice3='".mysqli_real_escape_string($price4)."',itemPrice4='".mysqli_real_escape_string($price5)."',
			itemPrice5='".mysqli_real_escape_string($price6)."',itemPrice6='".mysqli_real_escape_string($price7)."',
			itemPrice7='".mysqli_real_escape_string($price8)."',itemPrice8='".mysqli_real_escape_string($price9)."',
			itemPrice9='".mysqli_real_escape_string($price10)."',
			itemPrice10='".mysqli_real_escape_string($price11)."',
			itemPrice11='".mysqli_real_escape_string($price12)."',
			itemPrice12='".mysqli_real_escape_string($price13)."',
			itemPrice13='".mysqli_real_escape_string($price14)."',
			itemPrice14='".mysqli_real_escape_string($price15)."',
			itemPrice15='".mysqli_real_escape_string($price16)."',
			itemPrice16='".mysqli_real_escape_string($price17)."',
			itemPrice17='".mysqli_real_escape_string($price18)."',
			itemPrice18='".mysqli_real_escape_string($price19)."',
			itemPrice19='".mysqli_real_escape_string($price20)."',
			Cus_Pizza='".mysqli_real_escape_string($Cus_Pizza)."',
			Multiple_Toppings='".mysqli_real_escape_string($Multiple_Toppings)."',
			Free_Toppings='".mysqli_real_escape_string($Free_Toppings)."',
			Max_Toppings='".mysqli_real_escape_string($Max_Toppings)."',lastUpdatedOn=Now() WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['Item_ID']));

			$rec=mysqli_query($con, $sql);
			$Item_ID=sanitize($_REQUEST['Item_ID']);
		}
		else
		{
			$sql="INSERT INTO items(menuID,itemName,itemImage,itemDescription,itemStatus,popular,spicy,veggie,chef_special,itemPrice,itemPrice1,itemPrice2,itemPrice3,itemPrice4,itemPrice5,itemPrice6,itemPrice7,itemPrice8,itemPrice9,Cus_Pizza,Multiple_Toppings,Free_Toppings,Max_Toppings,lastUpdatedOn) VALUES('".mysqli_real_escape_string($con, sanitize($_REQUEST['Menu']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['itemname']))."','".$new_file_name."','".mysqli_real_escape_string($con, sanitize($_REQUEST['description']))."','".sanitize($_REQUEST['status'])."','".$popular."','".$spicy."','".$veggie."','".$chef_special."','".mysqli_real_escape_string($price1)."','".mysqli_real_escape_string($price2)."','".mysqli_real_escape_string($price3)."','".mysqli_real_escape_string($price4)."','".mysqli_real_escape_string($price5)."','".mysqli_real_escape_string($price6)."','".mysqli_real_escape_string($price7)."','".mysqli_real_escape_string($price8)."','".mysqli_real_escape_string($price9)."','".mysqli_real_escape_string($price10)."','".$Cus_Pizza."','".$Multiple_Toppings."','".$Max_Toppings."','".$Free_Toppings."',Now())";
			$rec=mysqli_query($con, $sql);
		...
		mysqli_query($con, $sql);
		$suboption=sanitize($_REQUEST['suboption']);
		$subgroup=sanitize($_REQUEST['subgroup']);
		$suboptionprice=sanitize($_REQUEST['suboptionprice']);
		$pp=1;
		for($i=0;$i<COUNT($suboption);$i+=1)
		{
			...
			if($suboption[$i]=='Pizza sauce')
				$orderid=1;
			if($suboption[$i]<>'') {
			$sql="INSERT INTO subitems(itemID,subgroup_id,subitemName,subitemPrice,Sel,status,lastUpdatedOn,sortOrder) VALUES(".$Item_ID.",'".mysqli_real_escape_string($subgroup[$i])."','".mysqli_real_escape_string(str_replace("\"", "&quot;",$suboption[$i]))."','".mysqli_real_escape_string($suboptionprice[$i])."',".sanitize($_REQUEST['suboptionsel'.$pp]).",'Active',Now(),".$orderid.")";
			mysqli_query($con, $sql);
			}
			$pp++;
		}
		...
		$sql="SELECT DISTINCT(id) AS MENU FROM menus WHERE merchantID=".sanitize($_REQUEST['Res_ID']);
		...
		$sql="UPDATE merchant SET Min_price=".$MINPRICE.", Max_price=".$MAXPRICE." WHERE id=".sanitize($_REQUEST['Res_ID']);
		mysqli_query($con, $sql);

		// Groups
		echo '<script language="javascript">location.href="dine.php?act=menu_item&Res_ID='.sanitize($_REQUEST['Res_ID']).'&Menu='.sanitize($_REQUEST['Menu']).'&rep=1";</script>';
		exit;
	}
	function duplicate_item()
	{
		...
		if($_REQUEST['Max_Toppings']=='')
			...
			$sql="INSERT Isanitize(NTO items(menuID,itemName,itemImage,itemDescription,itemStatus,popular,spicy,veggie,chef_special,itemPrice,itemPrice1,itemPrice2,itemPrice3,itemPrice4,itemPrice5,itemPrice6,itemPrice7,itemPrice8,itemPrice9,itemPrice10,itemPrice11,itemPrice12,itemPrice13,itemPrice14,itemPrice15,itemPrice16,itemPrice17,itemPrice18,itemPrice19,Cus_Pizza,Multiple_Toppings,Free_Toppings,Max_Toppings,lastUpdatedOn) VALUES('".mysqli_real_escape_string($con, sanitize($_REQUEST['Menu']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['itemname']))."','".$new_file_name."','".mysqli_real_escape_string($con, sanitize($_REQUEST['description']))."','".sanitize($_REQUEST['status'])."','".$popular."','".$spicy."','".$veggie."','".$chef_special."','".mysqli_real_escape_string($price1)."','".mysqli_real_escape_string($price2)."','".mysqli_real_escape_string($price3)."','".mysqli_real_escape_string($price4)."','".mysqli_real_escape_string($price5)."','".mysqli_real_escape_string($price6)."','".mysqli_real_escape_string($price7)."','".mysqli_real_escape_string($price8)."','".mysqli_real_escape_string($price9)."','".mysqli_real_escape_string($price10)."','".mysqli_real_escape_string($price11)."','".mysqli_real_escape_string($price12)."','".mysqli_real_escape_string($price13)."','".mysqli_real_escape_string($price14)."','".mysqli_real_escape_string($price15)."','".mysqli_real_escape_string($price16)."','".mysqli_real_escape_string($price17)."','".mysqli_real_escape_string($price18)."','".mysqli_real_escape_string($price19)."','".mysqli_real_escape_string($price20)."','".$Cus_Pizza."','".$Multiple_Toppings."','".$Max_Toppings."','".$Free_Toppings."',Now())";
	...
	function view_menus() {
		GLOBAL $con;
			$sql="SELECT * from menus WHERE Deleted=0 AND merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['Res_ID']));
			if($_REQUEST['Keyword']<>'')
			$sql.=" AND (menuName LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%' OR menuDescription LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%')";
	...
	function view_items() {
			GLOBAL $con;
			$sql="Select * from items WHERE Deleted=0 AND menuID=".mysqli_real_escape_string($con, sanitize($_REQUEST['Menu']));
			if($_REQUEST['Keyword']<>'')
			$sql.=" AND (itemName LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%' OR itemDescription LIKE '%".mysqli_real_escape_string($con, sanitize($_REQUEST['Keyword']))."%')";
	...
	function GetSubItemsGroupname($id) {
			GLOBAL $con;
			$sql="Select subitems.*,subgroups.subgroup_name,subgroups.id AS GID,subitems.id AS SID,subgroups.* from subitems INNER JOIN subgroups ON subgroups.id=subitems.subgroup_id WHERE itemID=".$id." AND subgroup_name NOT LIKE '%crust%' GROUP BY subgroup_id order by subgroup_id ASC";
			$result = mysqli_query($con, $sql);
			$TotalRecordCount=mysqli_num_rows($result);
			$i=0;
			$Record	=	array();
	...
	function GetSubItemsGroupnameItems($id,$groupid) {
				GLOBAL $con;
			$sql="Select subitems.*,subgroups.subgroup_name,subgroups.id AS GID,subitems.id AS SID from subitems INNER JOIN subgroups ON subgroups.id=subitems.subgroup_id WHERE itemID=".$id." AND subgroup_name NOT LIKE '%crust%' AND subgroup_id=".$groupid." order by subitemName ASC";
			$result = mysqli_query($con, $sql);
			$TotalRecordCount=mysqli_num_rows($result);
			$i=0;
			$Record	=	array();
	...
	{
		GLOBAL $con;
		$sql="Select * from delivery_fees WHERE merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']))." order by postalCode ASC";
		$result = mysqli_query($con, $sql);
		$TotalRecordCount=mysqli_num_rows($result);
		$i=0;
		$Record	=array();
	...
	function GeGetOpenCloseHours() {
			GLOBAL $con;
			 $sql="Select * from merchant_hours WHERE merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']))." order by id ASC";
			$result = mysqli_query($con, $sql);
			$TotalRecordCount=mysqli_num_rows($result);
			$i=0;
			$Record	=	array();
	...
	function Add_Menu()
	{
		...
		if($_REQUEST['party']=='')
			...
			$party=sanitize($_REQUEST['party']);
		if($_REQUEST['parentmenu']=='')
			$parentmenu=0;
		else
			$parentmenu=sanitize($_REQUEST['parentmenu']);
		if($_REQUEST['menuOrder']=='')
			$menuOrder=0;
		else
			$menuOrder=sanitize($_REQUEST['menuOrder']);

		if($_REQUEST['Menu_ID']>0)
		{
			$sql="UPDATE menus SET menuName='".mysqli_real_escape_string($con, sanitize($_REQUEST['menuname']))."',Main_Menu='".mysqli_real_escape_string($con, sanitize($_REQUEST['Main_Menu']))."',parentMenu='".mysqli_real_escape_string($parentmenu)."',menuDescription='".mysqli_real_escape_string($con, sanitize($_REQUEST['description']))."',Start_Time='".sanitize($_REQUEST['ST1'])."',End_Time='".sanitize($_REQUEST['ET1'])."',menuStatus='".sanitize($_REQUEST['status'])."',menuSize1='".mysqli_real_escape_string($con, sanitize($_REQUEST['size1']))."',menuSize2='".mysqli_real_escape_string($con, sanitize($_REQUEST['size2']))."',menuSize3='".mysqli_real_escape_string($con, sanitize($_REQUEST['size3']))."',menuSize4='".mysqli_real_escape_string($con, sanitize($_REQUEST['size4']))."',menuSize5='".mysqli_real_escape_string($con, sanitize($_REQUEST['size5']))."',menuSize6='".mysqli_real_escape_string($con, sanitize($_REQUEST['size6']))."',menuSize7='".mysqli_real_escape_string($con, sanitize( $_REQUEST['size7']))."',menuSize8='".mysqli_real_escape_string($con, sanitize($_REQUEST['size8']))."',menuSize9='".mysqli_real_escape_string($con, sanitize($_REQUEST['size9']))."',
			menuSize10='".mysqli_real_escape_string($con, sanitize($_REQUEST['size10']))."',
			menuSize11='".mysqli_real_escape_string($con, sanitize($_REQUEST['size11']))."',
			menuSize12='".mysqli_real_escape_string($con, sanitize($_REQUEST['size12']))."',
			menuSize13='".mysqli_real_escape_string($con, sanitize($_REQUEST['size13']))."',
			menuSize14='".mysqli_real_escape_string($con, sanitize($_REQUEST['size14']))."',
			menuSize15='".mysqli_real_escape_string($con, sanitize($_REQUEST['size15']))."',
			menuSize16='".mysqli_real_escape_string($con, sanitize($_REQUEST['size16']))."',
			menuSize17='".mysqli_real_escape_string($con, sanitize($_REQUEST['size17']))."',
			menuSize18='".mysqli_real_escape_string($con, sanitize($_REQUEST['size18']))."',
			menuSize19='".mysqli_real_escape_string($con, sanitize($_REQUEST['size19']))."',
			menuSize20='".mysqli_real_escape_string($con, sanitize($_REQUEST['size20']))."',
			menuLang1='".mysqli_real_escape_string($con, sanitize($_REQUEST['lang1']))."',menuLang2='".mysqli_real_escape_string($con, sanitize($_REQUEST['lang2']))."',menuLang3='".mysqli_real_escape_string($con, sanitize($_REQUEST['lang3']))."',pizza='".sanitize($_REQUEST['pizza'])."',menuOrder='".$menuOrder."',partyMenu='".$party."' WHERE id=".sanitize($_REQUEST['Menu_ID']);

		}
		else
		{
			$sql="INSERT INTO menus(merchantID,Main_Menu,parentMenu,menuName,menuDescription,Start_Time,End_Time,menuStatus,menuSize1,menuSize2,menuSize3,menuSize4,menuSize5,menuSize6,menuSize7,menuSize8,menuSize9,menuSize10,menuSize11,menuSize12,menuSize13,menuSize14,menuSize15,menuSize16,menuSize17,menuSize18,menuSize19,menuSize20,createdOn,menuLang1,menuLang2,menuLang3,pizza,partyMenu,menuOrder) VALUES('".sanitize($_REQUEST['Res_ID'])."','".mysqli_real_escape_string($con, sanitize($_REQUEST['Main_Menu']))."','".mysqli_real_escape_string($parentmenu)."','".mysqli_real_escape_string($con, $_REQUEST['menuname'])."','".mysqli_real_escape_string($con, $_REQUEST['description'])."','".$_REQUEST['ST1']."','".$_REQUEST['ET1']."','".$_REQUEST['status']."','".mysqli_real_escape_string($con, $_REQUEST['size1'])."','".mysqli_real_escape_string($con, $_REQUEST['size2'])."','".mysqli_real_escape_string($con, $_REQUEST['size3'])."','".mysqli_real_escape_string($con, $_REQUEST['size4'])."','".mysqli_real_escape_string($con, $_REQUEST['size5'])."','".mysqli_real_escape_string($con, $_REQUEST['size6'])."','".mysqli_real_escape_string($con, $_REQUEST['size7'])."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size8']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size9']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size10']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size11']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size12']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size13']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size14']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size15']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size16']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size17']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size18']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size19']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size20']))."',
			Now(),'".mysqli_real_escape_string($con, sanitize($_REQUEST['lang1']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['lang2']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['lang3']))."','".sanitize($_REQUEST['pizza'])."','".$party."','".$menuOrder."')";

		}

		$rec=mysqli_query($con, $sql);
		$menunames=sanitize($_REQUEST['menunames']);
		//echo $sql;exit;
		for($i=0;$i<COUNT($menunames);$i+=1)
		{

			if($menunames[$i]<>'') {
			$sql="INSERT INTO menus(merchantID,Main_Menu,parentMenu,menuName,menuDescription,Start_Time,End_Time,menuStatus,menuSize1,menuSize2,menuSize3,menuSize4,menuSize5,menuSize6,menuSize7,menuSize8,menuSize9,menuSize10,menuSize11,menuSize12,menuSize13,menuSize14,menuSize15,menuSize16,menuSize17,menuSize18,menuSize19,menuSize20,createdOn,menuLang1,menuLang2,menuLang3,pizza,partyMenu) VALUES('".sanitize($_REQUEST['Res_ID'])."','".mysqli_real_escape_string($con, sanitize($_REQUEST['Main_Menu']))."','".mysqli_real_escape_string($con,$parentmenu)."','".mysqli_real_escape_string($menunames[$i])."','".mysqli_real_escape_string($con, sanitize($_REQUEST['description']))."','".sanitize($_REQUEST['ST1'])."','".sanitize($_REQUEST['ET1'])."','".sanitize($_REQUEST['status'])."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size1']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size2']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size3']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size4']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size5']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size6']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size7']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size8']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size9']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['size10']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size8']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size9']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size10']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size11']))."',
			...
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size19']))."',
			'".mysqli_real_escape_string($con, sanitize($_REQUEST['size20']))."',
			Now(),'".mysqli_real_escape_string($con, sanitize($_REQUEST['lang1']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['lang2']))."','".mysqli_real_escape_string($con, sanitize($_REQUEST['lang3']))."','".sanitize($_REQUEST['pizza'])."','".$party."')";

			mysqli_query($con, $sql);
			}
		}
			echo '<script language="javascript">location.href="dine.php?act=my_menus&Res_ID='.sanitize($_REQUEST['Res_ID']).'&rep=1";</script>';
		exit;
	}
	function Settings_Restaurant()
	{
		...
		else
		{
			...
			$google_icon_file=sanitize($_REQUEST['google_icon']);
		}
		if($error=='') {
			$sql="UPDATE merchant SET logo='".$new_file_name."' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
			mysqli_query($sql);
		}
		if($mapiconerror=='') {
			$sql="UPDATE merchant SET google_icon='".$google_icon_file."' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
			mysqli_query($sql);
		}
		if(sanitize($_REQUEST['showTax'])=='')
			$showtax="No";
		else
			$showtax=mysqli_real_escape_string($con, sanitize($_REQUEST['showTax']));
		if($_REQUEST['tax']=='')
			$tax=0;
		else
			$tax=mysqli_real_escape_string($con, sanitize($_REQUEST['tax']));
		if($_REQUEST['minimumamount']=='')
			$minimumamount=0;
		else
			$minimumamount=mysqli_real_escape_string($con, sanitize($_REQUEST['minimumamount']));
		if($_REQUEST['deliveryfee']=='')
			$deliveryfee=0;
		else
			$deliveryfee=mysqli_real_escape_string($con, sanitize($_REQUEST['deliveryfee']));
		
		if(sanitize($_REQUEST['deliverymiles'])=='')
			$deliverymiles=0;
		else
			$deliverymiles=mysqli_real_escape_string($con, sanitize($_REQUEST['deliverymiles']));
		$sql="UPDATE merchant SET minimumDeliveryAmount='".$minimumamount."',deliveryFee='".$deliveryfee."',deliverymiles='".$deliverymiles."',deliveryareas='".mysqli_real_escape_string($con, sanitize($_REQUEST['deliveryareas']))."',deliveryzipcodes='".mysqli_real_escape_string($con, sanitize($_REQUEST['deliveryzipcodes']))."',delivery_basedon='".mysqli_real_escape_string($con, sanitize($_REQUEST['delivery_basedon']))."',deliveryWaitTime='".mysqli_real_escape_string($con, sanitize($_REQUEST['deliverywaittime']))."',takeout_time='".mysqli_real_escape_string($con, sanitize($_REQUEST['takeout_time']))."',showTax='".$showtax."',tax='".$tax."' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));

		mysqli_query($con, $sql);
		// Sub Groups
		if(sanitize($_REQUEST['SubGroup_Control'])==1)
		{
		$subgroups=sanitize($_REQUEST['subgroups']);
		$sbids=sanitize($_REQUEST['sbids']);
		$subgroups_required=sanitize($_REQUEST['subgroups_required']);
		$subgroups_multiple=sanitize($_REQUEST['subgroups_multiple']);
		$subids="";
		for($i=0;$i<COUNT($subgroups);$i+=1)
		{
			if($subgroups[$i]<>'') {
				if($sbids[$i]>0) {
					$sql="UPDATE subgroups SET subgroup_name='".mysqli_real_escape_string($subgroups[$i])."',required=".$subgroups_required[$i].",multiple=".$subgroups_multiple[$i]." WHERE id=".$sbids[$i];
					mysqli_query($con, $sql);
					$subids.=",".$sbids[$i];
				}
				else
				{
					$sql="INSERT INTO subgroups(merchant_id,subgroup_name,required,multiple) VALUES(".mysqli_real_escape_string($con, sanitize($_REQUEST['ID'])).",'".mysqli_real_escape_string($subgroups[$i])."',".$subgroups_required[$i].",".$subgroups_multiple[$i].")";
					mysqli_query($con, $sql);
		...
		$subids=trim($subids,",");
		$sql="DELETE FROM subgroups WHERE merchant_id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']))." AND id NOT IN (".$subids.")";
		mysqli_query($con, $sql);
		}
		...
		// Fees based on Miles
		$sql="DELETE FROM delivery_fees WHERE merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
		mysqli_query($con, $sql);

		$zonalfees_postcode=sanitize($_REQUEST['zonalfees_postcode']);

		$zonalfees_charge=sanitize($_REQUEST['zonalfees_charge']);
		$zonalfees_minfee=sanitize($_REQUEST['zonalfees_minfee']);

		for($i=0;$i<COUNT($zonalfees_postcode);$i+=1)
		{
			//".$zonalfees_minfee[$i];
			if($zonalfees_postcode[$i]<>'') {
			$sql="INSERT INTO delivery_fees(merchantID,postalCode,fees,minFee) VALUES(".mysqli_real_escape_string($con, sanitize($_REQUEST['ID'])).",'".$zonalfees_postcode[$i]."',".$zonalfees_charge[$i].",0)";
			mysqli_query($con, $sql);}
		}

		// Fees based on Zipcodes
		$sql="DELETE FROM deliveryzipcode_fees WHERE merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
		mysqli_query($con, $sql);

		$zonalfees_zipcode=sanitize($_REQUEST['zonalfees_zipcode']);
		$zonalfees_zipcodecharge=sanitize($_REQUEST['zonalfees_zipcodecharge']);

		for($i=0;$i<COUNT($zonalfees_zipcode);$i+=1)
		{
			if($zonalfees_zipcode[$i]<>'') {
			$sql="INSERT INTO deliveryzipcode_fees(merchantID,postalCode,fees) VALUES(".mysqli_real_escape_string($con, sanitize($_REQUEST['ID'])).",'".$zonalfees_zipcode[$i]."',".$zonalfees_zipcodecharge[$i].")";
			mysqli_query($con, $sql);}
		}

		// Open close time
		$sql="DELETE FROM merchant_hours WHERE merchantID=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
		mysqli_query($con, $sql);

		$bussinessclose=sanitize($_REQUEST['bussinessclose']);
		$start=sanitize($_REQUEST['start']);
		$end=sanitize($_REQUEST['end']);
		$start1=sanitize($_REQUEST['start1']);
		$end1=sanitize($_REQUEST['end1']);
		$deliveryclosed=sanitize($_REQUEST['deliveryclosed']);
		$deliverystart=sanitize($_REQUEST['deliverystart']);
		$deliveryend=sanitize($_REQUEST['deliveryend']);
		$deliverystart1=sanitize($_REQUEST['deliverystart1']);
		$deliveryend1=sanitize($_REQUEST['deliveryend1']);
		$pickupclosed=sanitize($_REQUEST['pickupclosed']);
		$pickupstart=sanitize($_REQUEST['pickupstart']);
		$pickupend=sanitize($_REQUEST['pickupend']);
		$pickupstart1=sanitize($_REQUEST['pickupstart1']);
		$pickupend1=sanitize($_REQUEST['pickupend1']);
		...
		if($error<>"")
			echo '<script>window.location="dine.php?act=res_settings&err=1&ID='.sanitize($_REQUEST['ID']).'";</script>';
		else if($mapiconerror<>"")
			echo '<script>window.location="dine.php?act=res_settings&merr=1&ID='.sanitize($_REQUEST['ID']).'";</script>';
		else
	...
	function Delete_menu()
	{
		...
		$qry="UPDATE menus SET Deleted=1,menuStatus='Inactive' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['Menu_ID']));

		   mysqli_query($con, $qry);
		  // Delete Menu Items
		  //$sql="DELETE FROM items WHERE menuID=".$_REQUEST['Menu_ID'];
		  $sql="UPDATE items SET Deleted=1,itemStatus='Inactive' WHERE menuID=".mysqli_real_escape_string($con, sanitize($_REQUEST['Menu_ID']));
		  mysqli_query($con, $sql);
		   echo '<script>window.location="dine.php?act=my_menus&Res_ID='.sanitize($_REQUEST['Res_ID']).'";</script>';
		   exit;
	}
	function Delete_menuitem()
	{
		GLOBAL $con;
			// $sql="DELETE FROM items WHERE id=".$_REQUEST['Item_ID'];
			  $sql="UPDATE items SET Deleted=1,itemStatus='Inactive' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['Item_ID']));
			mysqli_query($con, $sql);
		   echo '<script language="javascript">location.href="dine.php?act=menu_item&Res_ID='.sanitize($_REQUEST['Res_ID']).'&Menu='.sanitize($_REQUEST['Menu']).'";</script>';
		   exit;
	}
	function Delete_ItemImage()
	{
		...
		$sql="UPDATE items SET itemImage='' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['Item_ID']));
		mysqli_query($con, $sql);
		unlink("../admin/upload/users/".sanitize($_REQUEST['img']));
		echo '<script language="javascript">location.href="dine.php?act=add_item&Res_ID='.sanitize($_REQUEST['Res_ID']).'&Menu='.sanitize($_REQUEST['Menu']).'&Item_ID='.sanitize($_REQUEST['Item_ID']).'";</script>';
		exit;
	}
	function Delete_Restaurant()
	{
		GLOBAL $con;
		   ...
		   $qry="UPDATE merchant SET Deleted=1,status='Inactive' WHERE id=".mysqli_real_escape_string($con, sanitize($_REQUEST['ID']));
		   mysqli_query($con, $qry);
	...
	{
		GLOBAL $con;
		$id=mysqli_real_escape_string($con, sanitize($_REQUEST["id"]));
		...
		$book_date=date("Y-m-d",strtotime(sanitize($_REQUEST["book_date"])));
		$book_fulldate=date("Y-m-d G:i:s",strtotime(sanitize($_REQUEST["book_date"])." ".sanitize($_REQUEST["book_time"])));

		$meal=mysqli_real_escape_string($con, sanitize($_REQUEST["meal"]));
		$Start_Time=date("G:i:s",strtotime(sanitize($_REQUEST["book_time"])));
		// calculate End Time
		$End_Time=date("G:i:s",sanitize($_REQUEST["book_totime"]));
		$Look_Ahead=mysqli_real_escape_string($con, sanitize($_REQUEST["Look_Ahead"]));
		$Size=mysqli_real_escape_string($con, sanitize($_REQUEST["size"]));
		$Book_Name=mysqli_real_escape_string($con, sanitize($_REQUEST["Diner_Name"]));
		if($Book_Name=='First Name')
			$Book_Name="";

		$Book_LName=mysqli_real_escape_string($con, sanitize($_REQUEST["Diner_LName"]));
		if($Book_LName=='Last Name')
			$Book_LName="";
		if($Book_LName<>'')
			$Book_Name.=" ".$Book_LName;
		$Book_Email=mysqli_real_escape_string($con, sanitize($_REQUEST["Diner_Email"]));
		if($Book_Email=='For Reservation Confirmation')
			$Book_Email="";
		$Book_Phone=mysqli_real_escape_string($con, sanitize($_REQUEST["Diner_Phone"]));
		if($Book_Phone=='555-555-5555')
			$Book_Phone="";
		$Book_Phone_Contact=mysqli_real_escape_string($con, sanitize($_REQUEST["contact_principle"]));
		$Book_FirstTime=mysqli_real_escape_string($con, sanitize($_REQUEST["first_time"]));
		$Book_Notes=mysqli_real_escape_string($con, sanitize($_REQUEST["Special_Request"]));
		if($Book_Notes=='Please note that not all requests can be accommodated')
			$Book_Notes="";
		$special_events=sanitize($_REQUEST["special_events"]);
		...
		if($_SESSION['User_ID']=='')
		{
			...
			if($qnum>0)
			{
				$book_date1=date("m/d/Y",sanitize($_REQUEST["book_date"]));
				$book_time1=date("g:i a",sanitize($_REQUEST["book_time"]));
				echo "<script language='javascript'>location.href='table_confirm.php?id=".sanitize($_REQUEST['id'])."&err=email&book_date=".$book_date1."&meal=".sanitize($_REQUEST['meal'])."&size=".sanitize($_REQUEST['size'])."&book_time=".$book_time1."&book_totime=".sanitize($_REQUEST['book_totime'])."&Diner_Name=".sanitize($_REQUEST['Diner_Name'])."&Diner_Phone=".sanitize($_REQUEST['Diner_Phone'])."&Diner_Email=".sanitize($_REQUEST['Diner_Email'])."&lastname=".$Book_LName."&Special_Request=".$Book_Notes."';</script>";
				exit;
			}
			$length = 30;
			...
			$sql ="insert into tbl_registeration(email_add,password,firstname,display_name,mobile_phone,contact_principle,checksum_register,status,cdate) VALUES('".$Book_Email."','".$password."','".sanitize($_REQUEST['Diner_Name'])."','".sanitize($_REQUEST['Diner_Name'])."','".sanitize($_REQUEST['Diner_Phone'])."','Mobile','".$checksum_register."',0,now())";
			...
			$Subject="Welcome to Way.com, ".sanitize($_REQUEST['Diner_Name'])."! ";
					...
					<td  valign="top" scope="col"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					 <tr>
					  ...
					  <tr>
						...
						<tr>
							<td height="26" scope="col"><div align="left"><font style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#6a6a6a;">Hello</font><font style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#be3917;">&nbsp;&nbsp;'.sanitize($_REQUEST['Diner_Name']).'</font>
							</div></td>
						  </tr>
		...
		VALUES (".sanitize($_REQUEST['Res_UserID']).",'".$User_ID."',".$id.",'".$book_date."','".$book_fulldate."','".$meal."','".$Start_Time."','".$End_Time."','".$Size."','".$Book_Name."','".$Book_Email."','".$Book_Phone."','".$Book_Phone_Contact."','".$Book_FirstTime."','".$special_events."','".$Book_Notes."','".$code."',now())";

		mysqli_query($con, $sql);
		$Booking_ID=mysqli_insert_id($con);
		GLOBAL $adminmail,$Host_Path,$contactmail;

		$sql="SELECT email,contactName,merchantName,contactAddress,state,city,postalCode,telephone,faxNumber FROM merchant WHERE id=".sanitize($_REQUEST['id']);
		$exe=mysqli_query($con, $sql);
		$res=mysqli_fetch_array($exe);
	...
	function view_Reservations() {
			GLOBAL $con;
			$sql="Select tbl_tablebooking_bookings.*,tbl_registeration.* from tbl_tablebooking_bookings INNER JOIN tbl_registeration ON tbl_registeration.id=tbl_tablebooking_bookings.Book_UserID WHERE Book_Restaurant=".sanitize($_REQUEST['Res_ID'])." order by Book_Created desc";
			$result = mysqli_query($con, $sql);
			$TotalRecordCount=mysqli_num_rows($result);
			$Totalpages 		= mysqli_query($con, "SELECT * FROM `tbl_control`");
			$exec_paging		= mysqli_fetch_array($Totalpages);
	...
	{
		GLOBAL $con;
		$sql="SELECT Start_Time,End_Time FROM menus WHERE Main_Menu='".mysqli_real_escape_string($Menu)."' AND merchantID=".mysqli_real_escape_string($con, $_REQUEST['id']);
		$exe=mysqli_query($con, $sql) or die(mysqli_error($con));
	...
	function getMerchantOpenCloseStatus_Search($MID,$time,$orderType)
	{
		...
		$mexe=mysqli_query($con, $msql) or die(mysqli_error());
	...
	{
		GLOBAL $con;
		$sql="SELECT id,menuName,menuDescription,Start_Time,End_Time,menuSize1,menuSize2,menuSize3,menuSize4,menuSize5,menuSize6,menuSize7,menuSize8,menuSize9,menuSize10,menuSize11,menuSize12,menuSize13,menuSize14,menuSize15,menuSize16,menuSize17,menuSize18,menuSize19,menuSize20 FROM menus WHERE menuStatus='Active' AND merchantID=".$id." AND Main_Menu='".mysqli_real_escape_string($MainMenu)."' ORDER BY menuOrder,menuName";
		$exe=mysqli_query($con, $sql);
		$num=mysqli_num_rows($exe);
		$i=0;
		$record=array();
	...
	{
		GLOBAL $con;
		$query="SELECT Count(*) AS CNT FROM items INNER JOIN menus ON menus.id=items.menuID WHERE items.itemStatus='Active' AND menus.Main_Menu='".mysqli_real_escape_string($tab)."' AND menus.merchantID=".mysqli_real_escape_string($con, $_REQUEST['id']);
		if($type!='')
		{
			if($type=="Popular")
	...
	{
		GLOBAL $con;
		$sql="SELECT id,itemName,itemDescription,itemPrice,itemPrice1,itemPrice2,itemPrice3,itemPrice4,itemPrice5,itemPrice6,itemPrice7,itemPrice8,itemPrice9,itemPrice10,itemPrice11,itemPrice12,itemPrice13,itemPrice14,itemPrice15,itemPrice16,itemPrice17,itemPrice18,itemPrice19,popular,spicy,veggie,chef_special,itemImage,Cus_Pizza FROM items WHERE itemStatus='Active' AND menuID=".mysqli_real_escape_string($Menu_ID);
		if($_SESSION['CT']<>'')
		{
			$CTT=0;
	...
	function reservations()
	{
		...
		if($_REQUEST['Book_ID']<>'')
		{
			...
			$sq.= " AND orders.id='".mysqli_real_escape_string($Book_ID)."'";
		}
	...
	function CheckDeliveryDetails($User_ID,$Email,$street,$city,$state,$zipcode)
		{
			...
			if($num<1)
			{
				$sql="INSERT INTO tbl_deliveryaddress(DE_UserID,DE_Address,DE_State,DE_City,DE_Zipcode,DE_Created)VALUES(".$User_ID.",'".mysqli_real_escape_string($street)."','".mysqli_real_escape_string($state)."','".mysqli_real_escape_string($city)."','".mysqli_real_escape_string($zipcode)."',Now())";
				mysqli_query($con, $sql);
			}
