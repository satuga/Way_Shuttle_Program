...
if (isset($User_ID) && $User_ID != '') {
    ...
    /* Because of later payment option for parking */
    $ErrorMessage = 'Success! Pay later on the checkout time.';
    $OrderID = '000000';
    $TransactionID = '';
    $charge->paid = 1;
    $charge->id = '000000';
    ...
    if ($charge->paid == "1" && $charge->id != "") {
      ...
      ('" . sanitize($_REQUEST['MID']) . "'," . $result['Owner_ID'] . ",'" . mysqli_real_escape_string($con, $orderdate) . "','" . sanitize($_REQUEST['ordertime']) . "','" . $User_ID . "',NOW(),'" . mysqli_real_escape_string($con, sanitize($_REQUEST['orderfor'])) . "','" . mysqli_real_escape_string($con, sanitize($_REQUEST['orderAmount'])) . "','" . $tip . "','Confirmed','" . $paymentStatus . "','" . mysqli_real_escape_string($con, sanitize($_REQUEST['pay_type'])) . "','" . mysqli_real_escape_string($con, sanitize($_REQUEST['dine_tax'])) . "','" . mysqli_real_escape_string($con, sanitize($_REQUEST['discount'])) . "','" . $sess_points . "','" . $Points . "','" . $deliveryFee . "','" . mysqli_real_escape_string($con, sanitize($_REQUEST['comments'])) . "','" . mysqli_real_escape_string($con, $ResCC['Card_Type']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_FirstName']) . "',AES_ENCRYPT('" . $ResCC['CC_NUMBER'] . "','" . $EncryptKey . "'),'" . mysqli_real_escape_string($con, $ResCC['Card_Exp_Month']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_Exp_Year']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_Zip']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_Street']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_State']) . "','" . mysqli_real_escape_string($con, $ResCC['Card_City']) . "','" . $code . "','" . mysqli_real_escape_string($con, $Delivery_Address) . "','" . mysqli_real_escape_string($con, $Instructions) . "','" . sanitize($_REQUEST['delivery_lat']) . "','" . sanitize($_REQUEST['delivery_long']) . "','" . sanitize($_REQUEST['parking_total']) . "','" . sanitize($_REQUEST['dine_total']) . "')";
        ...
        ('" . $Order_Id . "','DINE','" . $User_ID . "',NOW(),'" . sanitize($_REQUEST['promo_id']) . "')";
            ...
            /* ========================================= End Bindra Changes ==================================================== */
            /**Start - Descrease spaces base on the parking id and from date*/
            $fromdateAvailability = sanitize($_REQUEST['fromDate']);
            $todateAvailability = sanitize($_REQUEST['toDate']);
            $parkingidForSpaces = sanitize($_REQUEST['id']);
            $parkingidMain = sanitize($_REQUEST['main_id']);
	    descreaseSpacesByPid($fromdateAvailability, $todateAvailability, $parkingidMain);
            //UpdateParkingQntyWithOrderID($Payment_Id);
            $data['beacons'] = getBeaconDetail($parkingidMain, 1);
            /**End - Descrease spaces base on the parking id and from date*/
            //exit;
            $content = array("status" => "1", "message" => "success", "data" => $data, "rep" => $rep, "resp" => $TransactionResult, "TxnID" => $Payment_Id, "Pay_TxnID" => $Order_Ids, "PTxnID" => $PTI_TID, "err" => $ErrorMessage);
...
function getBeaconDetail($garage_id, $gatemode = 1) {
    GLOBAL $con;
    $beacon = array();
