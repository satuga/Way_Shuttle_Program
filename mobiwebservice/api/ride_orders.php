...
$latitude = sanitize($data['data']['order_lat']);
$longitude = sanitize($data['data']['order_long']);
...
if($latitude != '' && $longitude != '')
{
    ...
    * cos((`delivery_lat`*pi()/180)) * cos((('$longitude'- `delivery_long`)*pi()/180))))*180/pi())*60*1.1515)
	...
	while($aRow=mysqli_fetch_array($res))
	{
		...
		$user_detail = "select firstname,lastname,home_phone,work_phone,mobile_phone,logo as customer_image from tbl_registeration where id = '".$aRow['customerID']."'";
		...
		while($res_info=mysqli_fetch_array($res_detail_info))
		{
			...
			//$info[$i]['tip'] = $aRow['tip']; // Add comment by bindra shah - 10-june-2016 Tips column already exist in db
			$info[$i]['tip'] = $aRow['Tips'];
      ...
      $info[$i]['customer_image'] =$dd[0]['customer_image'];
...
left join merchant as m ON m.id = o.merchantID where ride.driver_id = '".$driver_id."' and ride.status != '3' order by o.id desc";
	...
	while($rowss = mysqli_fetch_array($driver_query))
	{
		...
		//$records[$i]['tip'] = $rowss['tip']; // Add comment by bindra shah - 10-june-2016 Tips column already exist in db
		$records[$i]['tip'] = $rowss['Tips']; 
