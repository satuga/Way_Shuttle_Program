 <?php
    ...
    {
        $_REQUEST['toDate'] = date('m/d/Y', strtotime($_REQUEST['toDate']));
    	$dw = date('l', strtotime(sanitize($_REQUEST['toDate'])));
    	$ctime=date('g:i:A');
    	$sql="SELECT * FROM tbl_tablebooking_settings WHERE merchant=".sanitize($_REQUEST['id']);
    	...
    	if($num>0)
    	{
	    	...
	    	if(sanitize($_REQUEST['toDate'])==$today && $TBS_Breakfast_Start1<strtotime(date('H:i:s')))
	    	{
    	...
    	$wn = date('N', strtotime(sanitize($_REQUEST['toDate'])));
    	
    	if($wn==7)
    		$wn=0;
    	$sql="SELECT * FROM merchant_hours WHERE weekDay=".$wn." AND merchantID=".sanitize($_REQUEST['id']);
    	$exe=mysqli_query($con,$sql);
    	$res=mysqli_fetch_array($exe);
    	$num=mysqli_num_rows($exe);
    	if($res['closed']=='Yes')
    	...
    	else 
    	{
    		...
    		if(sanitize($_REQUEST['toDate'])==$today && $TBS_Breakfast_StartOther1<strtotime(date('H:i:s')))
    		{
    			$seconds = strtotime(date('H:i:s'));
    			$rounded = ceil($seconds / (5* 60)) * (5 * 60);
