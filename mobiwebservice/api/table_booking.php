...
if(isset($_REQUEST['id']) && isset($_REQUEST['book_date']) && isset($_REQUEST['book_time']) && isset($_REQUEST['size']) && isset($_REQUEST['Diner_Name'])
	...
	$Start_Time=date("g:i a",sanitize($_REQUEST["book_time"]));
	$book_fulldate=date("Y-m-d G:i:s",strtotime(sanitize($_REQUEST["book_date"])." ".$Start_Time));
	...
	if(sanitize($_REQUEST['User_ID'])=='')
	{
		...
		else
		{
			...
			$sql ="insert into tbl_registeration(email_add,encrypt_password,firstname,display_name,mobile_phone,contact_principle,checksum_register,status,cdate) VALUES('".$Book_Email."',AES_ENCRYPT('".$password."','".$EncryptKey."'),'".sanitize($_REQUEST['Diner_Name'])."','".sanitize($_REQUEST['Diner_Name'])."','".sanitize($_REQUEST['Diner_Phone'])."','Mobile','".$checksum_register."',1,now())";
