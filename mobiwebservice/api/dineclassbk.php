  ...
  Function Name: MergeShowCart
	...
	*/
	...
	function MergeShowCart(){
		...
		if($_REQUEST['user_id'] !=""){
		 $Query="SELECT tbl_cart.*,items.itemName,items.Cus_Pizza from tbl_cart INNER JOIN items ON items.id=tbl_cart.Cart_ServiceID where Cart_UserID='".$_REQUEST['user_id']."'";
		}else{
			$Query="SELECT tbl_cart.*,items.itemName,items.Cus_Pizza from tbl_cart INNER JOIN items ON items.id=tbl_cart.Cart_ServiceID where Sess_ID='".$_REQUEST['device_id']."'";
		}
	...
	function view_Restaurants() {
			...
			if($_REQUEST['Keyword']<>'')
			$sql.=" AND (merchantName LIKE '%".mysqli_real_escape_string($con, $_REQUEST['Keyword'])."%' OR merchantName LIKE '%".mysqli_real_escape_string(strtoupper($_REQUEST['Keyword']))."%' OR merchantName LIKE '%".mysqli_real_escape_string(strtolower($_REQUEST['Keyword']))."%' OR merchantName LIKE '%".mysqli_real_escape_string(ucwords($_REQUEST['Keyword']))."%' OR merchantName LIKE '%".mysqli_real_escape_string(ucfirst($_REQUEST['Keyword']))."%' OR contactName LIKE '%".mysqli_real_escape_string($con, $_REQUEST['Keyword'])."%' OR contactAddress LIKE '%".mysqli_real_escape_string($con, $_REQUEST['Keyword'])."%' OR crossStreet LIKE '%".mysqli_real_escape_string($con, $_REQUEST['Keyword'])."%')";
	...
	else
	{
		...
		if($_REQUEST['Item_ID']>0)
		{
			...
			Max_Toppings='".mysqli_real_escape_string($Max_Toppings)."',lastUpdatedOn=Now() WHERE id=".mysqli_real_escape_string($con, $_REQUEST['Item_ID']);
		...
		for($i=0;$i<COUNT($suboption);$i+=1)
		{
			...
			if($suboption[$i]=='Pizza sauce')
				$orderid=1;
			if($suboption[$i]<>'') {
			$sql="INSERT INTO subitems(itemID,subgroup_id,subitemName,subitemPrice,Sel,status,lastUpdatedOn,sortOrder) VALUES(".$Item_ID.",'".mysqli_real_escape_string($subgroup[$i])."','".mysqli_real_escape_string(str_replace("\"", "&quot;",$suboption[$i]))."','".mysqli_real_escape_string($suboptionprice[$i])."',".$_REQUEST['suboptionsel'.$pp].",'Active',Now(),".$orderid.")";
			mysqli_query($con, $sql);
			}
			$pp++;
		}
	...
	function Add_Menu()
	{
		...
		for($i=0;$i<COUNT($menunames);$i+=1)
		{
			...
			if($menunames[$i]<>'') {
			$sql="INSERT INTO menus(merchantID,Main_Menu,parentMenu,menuName,menuDescription,Start_Time,End_Time,menuStatus,menuSize1,menuSize2,menuSize3,menuSize4,menuSize5,menuSize6,menuSize7,menuSize8,menuSize9,menuSize10,menuSize11,menuSize12,menuSize13,menuSize14,menuSize15,menuSize16,menuSize17,menuSize18,menuSize19,menuSize20,createdOn,menuLang1,menuLang2,menuLang3,pizza,partyMenu) VALUES('".$_REQUEST['Res_ID']."','".mysqli_real_escape_string($con, $_REQUEST['Main_Menu'])."','".mysqli_real_escape_string($parentmenu)."','".mysqli_real_escape_string($menunames[$i])."','".mysqli_real_escape_string($con, $_REQUEST['description'])."','".$_REQUEST['ST1']."','".$_REQUEST['ET1']."','".$_REQUEST['status']."','".mysqli_real_escape_string($con, $_REQUEST['size1'])."','".mysqli_real_escape_string($con, $_REQUEST['size2'])."','".mysqli_real_escape_string($con, $_REQUEST['size3'])."','".mysqli_real_escape_string($con, $_REQUEST['size4'])."','".mysqli_real_escape_string($con, $_REQUEST['size5'])."','".mysqli_real_escape_string($con, $_REQUEST['size6'])."','".mysqli_real_escape_string($con, $_REQUEST['size7'])."','".mysqli_real_escape_string($con, $_REQUEST['size8'])."','".mysqli_real_escape_string($con, $_REQUEST['size9'])."','".mysqli_real_escape_string($con, $_REQUEST['size10'])."',
			'".mysqli_real_escape_string($con, $_REQUEST['size8'])."',
			'".mysqli_real_escape_string($con, $_REQUEST['size9'])."',
			'".mysqli_real_escape_string($con, $_REQUEST['size10'])."',
			'".mysqli_real_escape_string($con, $_REQUEST['size11'])."',
	...
	function Settings_Restaurant()
	{
		...
		for($i=0;$i<COUNT($subgroups);$i+=1)
		{
			if($subgroups[$i]<>'') {
				...
				{
					$sql="INSERT INTO subgroups(merchant_id,subgroup_name,required,multiple) VALUES(".mysqli_real_escape_string($con, $_REQUEST['ID']).",'".mysqli_real_escape_string($subgroups[$i])."',".$subgroups_required[$i].",".$subgroups_multiple[$i].")";
					mysqli_query($con, $sql);
