...
$Transactions=GetTotalRestaurantTransactions($User_ID,$id);
$bsql="SELECT AVG(txt0) AS RATING FROM tbl_reviews WHERE R_Type='Dine' AND list_id =".$id;
...
$rnum=mysqli_num_rows($rexe);
		if($rnum>0)
		{
			...
			{
				$asql="SELECT AVG(txt".$ki.") AS RATING FROM tbl_reviews WHERE R_Type='Dine' AND list_id =".$id;
