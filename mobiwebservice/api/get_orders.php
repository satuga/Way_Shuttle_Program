...
$page = sanitize($data['data']['page'])!='' ? sanitize(trim($data['data']['page'])):'1';
$status = sanitize($data['data']['status']); // current & previous
$monthCalendar = !empty($data['data']['monthCalendar'])?$data['data']['monthCalendar']:'';
$yearCalendar = !empty($data['data']['yearCalendar'])?$data['data']['yearCalendar']:'';
$responseCalendar = 0;
if(!empty($monthCalendar) && !empty($yearCalendar)){
    $limit = 500;
    $fullDate = $yearCalendar."-".$monthCalendar;
    $expiredate = date("Y-m", strtotime($fullDate));
    //echo $expiredate;exit;
    $paymentTranWhere = " WHERE date_format(to_date, '%Y-%m')='$expiredate'  AND (tbl_paymenttransaction.Item_ID<>'' or tbl_paymenttransaction.Parking_ID>0) AND  UsrID=$user_id";
    $ordersWhere = " WHERE date_format(orders.orderDate, '%Y-%m')='$expiredate' AND  orders.customerID=$user_id";
    $userWhere = " WHERE T_UsrID=$user_id";
    $bookingWhere = " WHERE date_format(Book_date, '%Y-%m') ='$expiredate'  AND Book_UserID=$user_id";
	$responseCalendar = 1;
} else {
$limit=20;
...
if($status == 'current')
{
	...
	$paymentTranWhere=" WHERE  OrderStatus ='Pending'  AND (tbl_paymenttransaction.Item_ID<>'' or tbl_paymenttransaction.Parking_ID>0) AND  UsrID=$user_id";
	//$ordersWhere=" WHERE  orders.orderDate>='$expiredate' AND orders.orderTime>='$expiretime'  AND  orders.customerID=$user_id";
    $ordersWhere=" WHERE (DATE_FORMAT(CONCAT(orders.orderDate, ' ', orders.orderTime), '%Y-%m-%d %H:%i:%s')>='$expire') AND  orders.customerID=$user_id";
	$userWhere=" WHERE T_UsrID=$user_id";
...
$query=$sql.$orderBy.$sqlLimit; //die;
//echo $query; die;
//$query=$sql;
...
if($TotalRecordCount > 0)
{
	while($aRow=mysqli_fetch_array($res))
	{
        ...
        if($v['Main']==1)
        {
            ...
            else if($v['Care_ID']>0)
            {
				...
				$contact[$i]['smart_lot'] = $Venue[7];
				$isParkingHasShuttle=isParkingHasShuttle($v["Parking_ID"]);
				$contact[$i]['parking_has_shuttle'] = $isParkingHasShuttle;
				$phone= $Venue[8];

				$getParkingLongLat = getParkingLongLat($v["Parking_ID"]);
                $longitude = !empty($getParkingLongLat['lon'])?$getParkingLongLat['lon']:'';
                $latitude = !empty($getParkingLongLat['lat'])?$getParkingLongLat['lat']:'';
            }
        ...
        else
        {
            ...
            $address= $rr['address'];
            $phone= $rr['phone'];
			$longitude = !empty($rr['geoLong'])?$rr['geoLong']:'';
            $latitude = !empty($rr['geoLat'])?$rr['geoLat']:'';
        }
	...
	//echo $v['Main'];
		...
		$contact[$i]['res_lat'] = $latitude;
        $contact[$i]['res_long'] = $longitude;
	...
	{
		if($responseCalendar){
            $contact = replaceResponseBasedOnCalendar($contact);
        }
...
function replaceResponseBasedOnCalendar($contact){
    ...
    for ($i = 1;$i<=31;$i++){
        ...
        foreach ($contact as $records){
                $from_date = date('d', strtotime($records['from_date']));
                if($i == $from_date){
                    $response['date'] = $i;
                    $response['moredetail'][] = $records;
                }
        }
        if(!empty($response)){
    ...
    //$globalResponse = array_filter($globalResponse);
