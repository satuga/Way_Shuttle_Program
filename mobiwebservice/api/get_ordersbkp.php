...
$sql1 = "SELECT tbl_registeration.firstname AS Name,tbl_registeration.email_add,tbl_registeration.voucher,tbl_registeration.logo as customer_image,PaymentSource,Item_ID,TxnID,code,TotalAmount,UsrID,PaymentDesc,Voucher_File,tbl_paymenttransaction.Status,date_format( tbl_paymenttransaction.TxnDate,'%d/%m/%Y') as regdate,tbl_paymenttransaction.TxnDate AS TxnDate,1 AS Main,tbl_paymenttransaction.Activity_ID,tbl_paymenttransaction.Movie_ID,tbl_paymenttransaction.Care_ID,tbl_paymenttransaction.Parking_ID,tbl_paymenttransaction.Ticket_ID,tbl_paymenttransaction.Cat_ID,tbl_paymenttransaction.OrderStatus,DeliveryMethod,0 as DeliveryLat,0 as DeliveryLong,orderType,Redeem,TxnDate AS ORDERCREATED,Ticket_Type AS Ticket_Type,Ticket_Quantity AS Ticket_Quantity,quantity,Dine_ID AS Dine_ID,from_date,to_date,0 AS orderDate,0 AS orderTime,tbl_paymenttransaction.Ticket_Title,Movie_Tickets,Instant_MovieTicket,tbl_paymenttransaction.Payat_Lot FROM tbl_paymenttransaction
			INNER JOIN tbl_registeration ON  tbl_registeration.id=tbl_paymenttransaction.UsrID
			WHERE Item_ID is not NULL AND UsrID=".$user_id.$dd;
$sql2 = "SELECT tbl_registeration.firstname AS Name,tbl_registeration.email_add,tbl_registeration.voucher,tbl_registeration.logo as customer_image,orders.paymentType AS PaymentSource,merchantID AS Item_ID,orders.id AS TxnID,code,orders.orderAmount AS TotalAmount,customerID AS UsrID,comments AS PaymentDesc,Voucher_File,orders.paymentStatus AS Status,date_format(orders.orderDate,'%d/%m/%Y') as regdate,orders.orderPlacedOn AS TxnDate,2 AS Main,0 AS Activity_ID,0 AS Movie_ID,0 AS Care_ID,0 AS Parking_ID,0 AS Ticket_ID,72 AS Cat_ID,orders.OrderStatus,DeliveryMethod,delivery_lat as DeliveryLat,delivery_long as DeliveryLong,orderType,Redeem,orderPlacedOn AS ORDERCREATED,0 AS Ticket_Type,0 AS Ticket_Quantity,GiftCard_QTY AS quantity,GiftCard_ID AS Dine_ID,orderDate AS from_date,0 AS to_date,orderDate,orderTime,0 AS Ticket_Title,0 AS Movie_Tickets,0 AS Instant_MovieTicket,0 AS Payat_Lot FROM orders
	INNER JOIN tbl_registeration ON  tbl_registeration.id=orders.customerID
	WHERE customerID=".$user_id.$dd;
$sql3 = "SELECT tbl_registeration.firstname AS Name,tbl_registeration.email_add,tbl_registeration.voucher,tbl_registeration.logo as customer_image,PaymentSource,T_Package AS Item_ID,tbl_packagetransaction.T_ID AS TxnID,code,T_TotalAmount AS TotalAmount,T_UsrID AS UsrID,PaymentDesc,0 AS Voucher_File,0 AS Status,date_format( tbl_packagetransaction.T_Created,'%d/%m/%Y') as regdate,tbl_packagetransaction.T_Created AS TxnDate,3 AS Main,0 AS Activity_ID,0 AS Movie_ID,0 AS Care_ID,0 AS Parking_ID,0 AS Ticket_ID,0 AS Cat_ID,OrderStatus,DeliveryMethod,0 as DeliveryLat,0 as DeliveryLong,'Package' AS orderType,Redeem,tbl_packagetransaction.T_Created AS ORDERCREATED,0 AS Ticket_Type,0 AS Ticket_Quantity,0 AS quantity,0 AS Dine_ID,'0000-00-00 00:00:00' AS from_date,0 AS to_date,0 AS orderDate,0 AS orderTime,0 AS Ticket_Title,0 AS Movie_Tickets,0 AS Instant_MovieTicket,0 AS Payat_Lot FROM tbl_packagetransaction
	INNER JOIN tbl_registeration ON  tbl_registeration.id=tbl_packagetransaction.T_UsrID
	WHERE T_UsrID=".$user_id.$dd;
$sql4 = "SELECT tbl_registeration.firstname AS Name,tbl_registeration.email_add,tbl_registeration.voucher,tbl_registeration.logo as customer_image,0 AS PaymentSource,Book_Restaurant AS Item_ID,tbl_tablebooking_bookings.Book_ID AS TxnID,code,0 AS TotalAmount,Book_UserID AS UsrID,Book_Notes AS PaymentDesc,0 AS Voucher_File,0 AS Status,date_format(tbl_tablebooking_bookings.Book_Created,'%d/%m/%Y') as regdate,tbl_tablebooking_bookings.Book_Created AS TxnDate,4 AS Main,0 AS Activity_ID,0 AS Movie_ID,0 AS Care_ID,0 AS Parking_ID,0 AS Ticket_ID,72 AS Cat_ID,0 AS OrderStatus,0 AS DeliveryMethod,0 as DeliveryLat,0 as DeliveryLong,0 AS orderType,Redeem,Book_Created AS ORDERCREATED,0 AS Ticket_Type,0 AS Ticket_Quantity,Book_Size AS quantity,Book_Restaurant AS Dine_ID,Book_date AS from_date,0 AS to_date,Book_date AS orderDate,Book_Start_Time AS orderTime,0 AS Ticket_Title,0 AS Movie_Tickets,0 AS Instant_MovieTicket,0 AS Payat_Lot FROM  tbl_tablebooking_bookings
	INNER JOIN tbl_registeration ON  tbl_registeration.id=tbl_tablebooking_bookings.Book_UserID
	WHERE Book_Restaurant is not NULL AND Book_UserID=".$user_id.$dd;
...
if($TotalRecordCount > 0)
{
	...
	{
		if ($aRow[Item_ID] != '' ){
		$rr = getrestaurantinfo($aRow['Item_ID'],$cat);
		//var_dump($rr);
		if($aRow['Parking_ID'] != '0'){
			// $cat = 45; for parking
			$whereServiceId = $aRow['Parking_ID'];
			$titleAddress = GetServiceTitle($aRow['Parking_ID'],45);
			//var_dump($titleAddress); die;
			$contact[$i]['title'] = $titleAddress[0];
			$contact[$i]['address'] =$titleAddress[1];
		}else{
			// $cat = 72; for dinning
			/*if($aRow['Item_ID']!= ''){	
			$titleAddress = GetServiceTitle($aRow['Item_ID'],72);
			} else {
			
		} */
			$contact[$i]['title'] = $rr['title'];	
		...
		//$contact[$i]['title'] = GetServiceTitle($whereServiceId,$cat);
		//$contact[$i]['address'] = $rr['address'];
		...
		if($contact[$i]['Parking_ID'] != "0"){
			$contact[$i]['type'] = "Parking";
		}else{
			$contact[$i]['type'] = "Dine";
		}
