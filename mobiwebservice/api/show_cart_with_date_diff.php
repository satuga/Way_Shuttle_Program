        ...
        foreach($cart['parking'] as $park_traverse_data){
          ...
          if($park_traverse_data['Payment_Type']=='partial'){
            ...
            //    print_r($get_loc); die;
                ...
                {
					$PriceArray=array();
					$endDate = new DateTime($park_traverse_data['to_date']);
					$fromDate = new DateTime($park_traverse_data['from_date']);
					$interval = $endDate->diff($fromDate);
					...
					$fdate =date_format($fromDate, 'Y-m-d'); 
                    $tdate = date_format($endDate, 'Y-m-d'); 
					$elapsed = $interval->format('%y years %m months %a days %h hours %i minutes %S seconds');
					$temp_array['days']=$days;
					/*$temp_array['hours']=$hours;
					$temp_array['minutes']=$minutes;
					$temp_array['endDate']=$endDate;*/					
					$temp_array['fdate']=$fdate;
					$temp_array['tdate']=$tdate;
                ...
                if(is_array($price))
				{
                   	$temp_array['Totaldays'] =count($PriceArray);
...
//
			...
			if(is_array($price))
			{
				$park_total+= number_format(($Dayprice+$Weekendprice),2);
				$temp_array['total']=number_format(($Dayprice+$Weekendprice),2) + $totalfees;				
			}
			else
			{
