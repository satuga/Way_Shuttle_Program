...
if(isset($_REQUEST['firstname']) && isset($_REQUEST['lastname']) && isset($_REQUEST['userid']))
{
    ...
    // Check for a correct extension. The image file hasn't an extension? Add one
       ...
       if($validation_type == 1)
       {
    	 $file_info = getimagesize($_FILES['image_file']['tmp_name']);
          ...
          $content=array("response"=>ERROR,"message"=>"The uploaded file doesn't seem to be an image.");
       ...
       else if($validation_type == 2)
       {
          ...
          $content=array("response"=>ERROR,"message"=>"You must upload a file with one of the following extensions: ".$exts);
            echo json_encode($content);
            exit;
    	  }
    ...
    $sql ="update tbl_registeration set firstname='".mysqli_real_escape_string($con,$con,$firstname)."',lastname='".mysqli_real_escape_string($con,$lastname)."',logo='".$new_file_name."' where id=".$userid;
    $rec=mysqli_query($con,$sql);
    if($rec){
        $content=array("response"=>SUCCESS,"message"=>SUCCESS,"file"=>IMAGEPATH.$new_file_name);
        echo json_encode($content);
        exit;
    }
    else{
