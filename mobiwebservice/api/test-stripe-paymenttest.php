...
if (isset($_REQUEST["User_ID"])&& $_REQUEST["User_ID"]!='')
{
	...
	if($charge->paid == "1" && $charge->id != "")
	{
		...
		// $Query="SELECT SUM(Cart_TotalAmount) AS TAMT,Cart_Package from tbl_cartpackages where Cart_Session='".$_REQUEST['device_id']."'";
		 $Query="SELECT SUM(Cart_TotalAmount) AS TAMT,Cart_Package from tbl_cartpackages where 	Cart_UserID='".$_REQUEST['User_ID']."'";


		$res = mysqli_query($con,$Query);
		$PRes1=mysqli_fetch_array($res);
		// $Query="SELECT * FROM tbl_cartpackages WHERE Cart_Session='".$_REQUEST['device_id']."'";
	  $Query="SELECT * FROM tbl_cartpackages WHERE Cart_UserID='".$_REQUEST['User_ID']."'";
		$res = mysqli_query($con,$Query);
		...
		$Pay_TxnID=0;
/* ================================================================================================================ */
    $Main_query="select * from tbl_cart where Cart_UserID='".$_REQUEST['User_ID']."'";
    $Main_res = mysqli_query($con,$Main_query);
    $AllTran=mysqli_num_rows($Main_res);
    $Dine_Query="select * from tbl_cart where Cart_UserID='".$_REQUEST['User_ID']."' AND Cart_Type='Dine'";
		$Dine_res = mysqli_query($con,$Dine_Query);
		//$result=mysqli_fetch_array($Dine_res);
		$DineTran=mysqli_num_rows($Dine_res);

    $Other_Query="select * from tbl_cart where Cart_UserID='".$_REQUEST['User_ID']."' AND Cart_Type!='Dine'";
    ...
    if($AllTran>0)
    {
     ...
     $Query="INSERT INTO orders (merchantID,Owner_ID,orderDate,orderTime,customerID,orderPlacedOn,orderType,orderAmount,tip,orderStatus,paymentStatus,paymentType,orderTaxAmount,discount,discount_points,points_earned,deliveryFee,comments,Card_Type,Card_Name,Card_Number,Card_Expiry_Month,Card_Expiry_Year,Card_Zip,Card_Address,Card_State,Card_City,code,DeliveryAddress,PaymentDesc,delivery_lat,delivery_long,parking_total,dine_total)
      ...
      if($DineTran>0)
      {
          while ($result11=mysqli_fetch_array($Dine_res))
          {
            // Insert Order Items
     ...
     /*  $queryemp="DELETE FROM tbl_cart where Cart_UserID='".$_REQUEST['User_ID']."'";
        mysqli_query($con,$queryemp);*/

       echo $sql="INSERT INTO tbl_payment(Dine_TxnID,Pay_TxnID,Package_TxnID,UsrID,Pay_TotalAmount,Pay_Created,Pay_Status) VALUES('".$Dine_TxnID."','".$Order_Ids."','".$PTI_TID."','".$User_ID."','".$Pay_TotalAmount."',NOW(),1)";
        mysqli_query($con,$sql);
    ...
    /*======================================================= Bindra Changes  ================================================================= */
                        ...
                        if($Transactions[4]!='')
                        {
                            ...
                            if($DineRecords[1]>0)
                            {
                                ...
                                foreach($DineRecords[0] as $k=>$v)
                                {
                                    ...
                                    if(COUNT($TxnIDs)>1) {
                                        $orderAmount=GetOrderAmount($OrderNo);
                                        $orderTaxAmount=GetOrdertaxAmount($OrderNo);
                                        $discount=GetOrderdiscountAmount($OrderNo);
                                        $deliveryFee=GetOrderdeliveryFee($OrderNo);
                                        $Tips=$v["Tips"];
                                    } else {
                                ...
                                $data['dine']['total']=number_format($orderAmount,2);
                                $grand_total+= number_format($orderAmount,2);
                ...
                if($Transactions[0]!='')
                {
                        ...
                        foreach($Records[0] as $k=>$v) {
                            ...
                            $totalDiscount+=$Discount;
                             if($v['Parking_ID']>0) {
                                ...
                                $tax = getMerchantTaxRate($v['Parking_ID']);
                              $OverAllTotalAmount+=$v['TotalAmount'];
                                $total_tax+= round((($v['TotalAmount'])*$tax/100),2);
                                $Order_No="PA".$v['TxnID'];
                                $temp_array['park_name']=$title;
                                $temp_array['order_id']=$Order_No;
                                $temp_array['type']='Parking';
                              $temp_array['amount'] =$v['TotalAmount'];
                    ...
                    $data['parking']['total_tax'] = $total_tax;
                    $data['parking']['discount'] = $totalDiscount;
                    $data['parking']['total'] = $OverAllTotalAmount + $total_tax;
                    $grand_total+= $data['parking']['total'];
                }    // End other Details
                ...
                if($_REQUEST['promo_id'] != '')
          {
           ...
           $promo_upsql="SELECT * from tbl_promocode WHERE PMC_PromoID = $promo_id AND PMC_IsActive = '1' LIMIT 1";
            ...
            $data['promocode_discount']=$promocode_discount;
            $data['promocode']=$promocode;
            $data['sub_total']=$grand_total;
            $data['grand_total']=$grand_total-$promocode_discount;
            $data['discount']=$data['dine']['discount']+$data['parking']['discount'];
            $data['earned_way_bucks']=$points_earned;
            $data['total_way_bucks']=GetAvailablePoints($_REQUEST['User_ID']);
    /* ========================================= End Bindra Changes ==================================================== */
