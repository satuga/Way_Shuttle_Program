    ...
    if (isset($Cart[0]) && !empty($Cart[0])) {
        ...
        if (isset($cart['dine']) && !empty($cart['dine'])) {
            ...
            foreach ($cart['dine'] as $dine_traverse_data) {
                ...
                $getMerchantInformation = getMerchantInformation($dine_traverse_data['Owner_Restaurant']);
                $main_cart['dine']['dine_name'] = $getMerchantInformation['merchantName'];
                $query_img = "SELECT items.itemImage FROM items
				  INNER JOIN menus ON menus.id=items.menuID
				  WHERE items.itemImage<>'' AND itemStatus='Active' AND menus.merchantID='" . $dine_traverse_data['Owner_Restaurant'] . "' ORDER BY ind_image_featured DESC LIMIT 1";
                ...
                $main_cart['dine']['dine_image'] = $rest_logo;
                $main_cart['dine']['Delivery_Available'] = $getMerchantInformation['delivery'];
                $main_cart['dine']['Pickup_Available'] = $getMerchantInformation['takeout'];
            ...
            $main_cart['dine']['dine_total'] = $subtotal;

            $today = strtotime(date("Y-m-d"));
            $Selecttime = strtotime('30 minutes', strtotime(date("F j, Y, g:i a")));
            $oneday = 24 * 3600;
            $Owner_Restaurant = $main_cart['dine']['items'][0]['Owner_Restaurant'];
            for ($i = $today; $i <= $today + 4 * $oneday; $i += $oneday) {
                ...
                if ($closed == 'No') {
                    ...
                    $main_cart['dine']['currentdatetime'] = date('Y-m-d g:i A');
                    $main_cart['dine']['openingTimes'] = $Open_Times;
                    $main_cart['dine']['pickupdate'] = $pickupDate;
                    ...
                    $main_cart['dine']['pickupDisplayDate'] = $label;
                    $main_cart['dine']['pickuptime'] = $pickupTime;
                    break;
                }
            }
        } else {
            $main_cart['dine'] = (object) array();
        }
        //end if of dine
        ...
        if (isset($cart['parking']) && !empty($cart['parking'])) {
            ...
            foreach ($cart['parking'] as $park_traverse_data) {
                ...
                if (is_array($price)) {
                    ...
                    $temp_array['price_rate'][]['price'] = strtolower($price);
...
//
            ...
            if (!empty($main_cart['parking'])) {
                $main_cart['parking'] = setParkingArray($main_cart['parking']);
            }
        ...
        $main_cart['paynow_actual'] = $main_cart['total_amount'] - $balance_due;
        $total_cart = 0;
        if ($user_id != "") {
...
function getMerchantInformation($rest) {
    GLOBAL $con;
    $rescity = mysqli_fetch_array(mysqli_query($con, "select delivery, takeout, merchantName from merchant where id=" . $rest));
    return $rescity;
}

function setParkingArray($args) {
    $tmp = array();
    foreach ($args as $arg) {
        $tmp[$arg['park_name']][] = $arg;
    }
    ...
    foreach ($tmp as $type => $labels) {
        $output[] = array(
            'park_name' => $type,
            'park_image' => $labels[0]['parking_image'],
            'park_detail' => $labels
        );
    }
    return $output;
