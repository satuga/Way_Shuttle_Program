...
if (!empty($rowCount)) {
    ...
    $mailto = !empty($_GET['emailto']) ? $_GET['emailto'] : 'ujash.letsnurture@gmail.com';
	 //$mailto = !empty($_GET['emailto']) ? $_GET['emailto'] : 'jeba.raj@way.com';
	  //$mailto = !empty($_GET['emailto']) ? $_GET['emailto'] : 'binu.girija@way.com, bhumi.bhutani@way.com';
    ...
    sendmail_1Hour($mailto, $subject, $message, $updateParkingIds);
}


function sendmail_1Hour($mailto, $subject, $message, $updateParkingIds) {
    GLOBAL $con;
    ...
    if(mail($mailto,$subject, $message, $headers)){
        /*$query = "update tbl_shuttle_active_status set SAS_ShuttleActive_LastTime_1Hour = '".date('Y-m-d H:i:s')."' where SAS_PatkingID IN (".$updateParkingIds.")";
        $res = mysqli_query($con, $query) or die("Error");*/
    }
}
