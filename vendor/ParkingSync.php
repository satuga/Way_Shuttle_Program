...
function UpdateParkingMapping($value)
   {    
            ...
            $newparkingId = isset($parkID)?$parkID:'';
            $parkingName = isset($listingName)?$listingName:'';
          ...
          if($newparkingId == '')
        {
            $results=array("status"=>"0","message"=>"Missing new parking id.");
            echo json_encode($results);
            exit;
        }  else if($parkingName == '')
        {
            $results=array("status"=>"0","message"=>"Missing parking name.");
            echo json_encode($results);
            exit;
        }   else if($lat == '' || $long == '')
        {
            $results=array("status"=>"0","message"=>"Missing longitude or latitude.");
            echo json_encode($results);
            exit;
        } else{  
            ...
            GLOBAL $con;            
            if (Empty($parkingId)) { 
                        echo $sql ="insert into tbl_parkinglocations set Park_Name='".$parkingName."',
                                                                    Park_UserID='".$createdUserID."',
                                                                    lat='".$lat."',
                                                                    lon='".$long."'";                   
                        $rec=mysqli_query($con, $sql);                        
                        $id=mysqli_insert_id($con);
                        $sql1 ="insert into tbl_parking set P_Location='".$id."', P_Status='".$active."'";
                        ...
                        $query=mysqli_query($con,"select `P_Location` from tbl_parking where P_ID =".$parkingId);
                        $num = mysqli_num_rows($query);
                        if($num > 0){
                            $results = mysqli_fetch_assoc($query);
                            $pid = $results['P_Location'];
                            $sql ="update tbl_parkinglocations set Park_Name='".$parkingName."',
                                                                        Park_UserID='".$createdUserID."',
                                                                        lat='".$lat."',
                                                                        lon='".$long."' where Park_ID = '".$pid."'";
                            $rec=mysqli_query($con, $sql);
                            $sql ="update tbl_parking set P_Status='".$active."' where P_ID = '".$map_userID."'";
                            $rec=mysqli_query($con, $sql);
                            ...
                            $output=array("status"=>"1","message"=>"Data not found.");
                            echo json_encode($output);
                            exit;
                        }
            }  
...
function loginAPI(){
    ...
    $url = "http://stgnew.way.com/way-service/security/login";
    $headers = array( 
...
function ParkingAPI($token){
    ...
    if($token){
            ...
            $parking_url = "http://138.68.197.125:8080/way-service/parkingListings/newAndModified?from=".$day_before."&to=".$date."&serviceName=Parking";
