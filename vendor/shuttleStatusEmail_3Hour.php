...
$query = "SELECT tsas.*, pl.Park_Name FROM tbl_shuttle_active_status tsas "
        ...
        . " where SAS_ShuttleActive = '0' and SAS_ShuttleActive_LastTime_3Hour <= DATE_SUB(NOW(), INTERVAL 3 HOUR)";
...
if (!empty($rowCount)) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        if (!empty($getSelectedVendorEmail)) {
            sendVendorEmail($getSelectedVendorEmail, $row['SAS_PatkingID']);
        }
    }
}
...
function sendVendorEmail($mailto, $updateParkingId) {
    GLOBAL $con;
...
<strong>Please do not reply to this email</strong><br/> ";
    ...
    $query = "update tbl_shuttle_active_status set SAS_ShuttleActive_LastTime_3Hour = '" . date('Y-m-d H:i:s') . "' where SAS_PatkingID IN (" . $updateParkingId . ")";
    $res = mysqli_query($con, $query) or die("Error");
}
