...
$Phone = $_REQUEST['Phone'];               ///---- do not send
$BLDG_No = $_REQUEST['BLDG_No'];           ///---- do not send

$pay_type = $_REQUEST['pay_type'];

$Card_Number1 = $_REQUEST["Card_Number"];
...
$Pending_Amount = $_REQUEST['Pending_Amount'];

$DeliveryAddress1 = $_REQUEST['DeliveryAddress'];
...
{	
	...
	$rexe=mysql_query($rsql);
	$rres=mysql_fetch_array($rexe);
	if($rres['mobile_phone']=='' || $rres['firstname']=='' || $rres['street']=='' || $rres['city']=='' || $rres['state']=='' || $rres['zipcode']=='')
	{
		$rsql="UPDATE tbl_registeration SET firstname='".mysql_real_escape_string($CC_First_Name)."',street='".mysql_real_escape_string($Street_Address)."',city='".mysql_real_escape_string($City)."',state='".mysql_real_escape_string($State)."',zipcode='".mysql_real_escape_string($Zip_Code)."',mobile_phone='".mysql_real_escape_string($Phone)."',BLDG_No='".mysql_real_escape_string($BLDG_No)."' WHERE id=".$user_id;
		mysql_query($rsql);
	}
	...
	if($ResCCNum<1)
	{
		$sql="INSERT INTO tbl_creditcarddetails(Card_User_ID,Card_Type,Card_Number,Card_Exp_Month,Card_Exp_Year,Card_FirstName,Card_Street,Card_City,Card_State,Card_Zip,Card_Created)VALUES(".$user_id.",'".mysql_real_escape_string($payment_method)."',AES_ENCRYPT('".mysql_real_escape_string($Card_Number)."','".$EncryptKey."'),'".mysql_real_escape_string($exp_month)."','".mysql_real_escape_string($exp_date)."','".mysql_real_escape_string($CC_First_Name)."','".mysql_real_escape_string($Street_Address)."','".mysql_real_escape_string($City)."','".mysql_real_escape_string($State)."','".mysql_real_escape_string($Zip_Code)."',Now())";
	...
	else
	{
		...
		$sql="UPDATE tbl_creditcarddetails SET Card_Type='".mysql_real_escape_string($payment_method)."',Card_Number=AES_ENCRYPT('".mysql_real_escape_string($Card_Number)."','".$EncryptKey."'),Card_Exp_Month='".mysql_real_escape_string($exp_month)."',Card_Exp_Year='".mysql_real_escape_string($exp_date)."',Card_FirstName='".mysql_real_escape_string($CC_First_Name)."',Card_Street='".mysql_real_escape_string($Street_Address)."',Card_State='".mysql_real_escape_string($State)."',Card_City='".mysql_real_escape_string($City)."',Card_Zip='".mysql_real_escape_string($Zip_Code)."' WHERE Card_ID=".$Card_ID;
		//mysql_query($sql);
	}
	...
	if($pay_type=='CreditCard')
	{
		...
		// Billing Address
		$Billing_Name=mysql_real_escape_string($CC_First_Name);
		$Billing_Address=mysql_real_escape_string($Street_Address);
		$Billing_City=mysql_real_escape_string($City);
		$Billing_State=mysql_real_escape_string($State);
		$Billing_Zip=mysql_real_escape_string($Zip_Code);
	...
	if($TransactionResult=='APPROVED' && $TransactionID>0)
	{
		...
		while ($result=mysql_fetch_array($res)) 
		{
			...
			//--------- Points Calculation ------------//
			$Points=0;
			$resvenue = @mysql_fetch_array(mysql_query("SELECT Point_Percentage FROM tbl_categories WHERE Cat_ID=".$Cat_ID));
	...
	else
	{
		...
		if($Payment_Id=='')
			$Payment_Id=0;
		$sql="INSERT INTO tbl_firstdatatransaction(TxnID,UserID,Amount,CardType,CardName,CardNumber,Card_Epx_Month,Card_Exp_Year,Card_Address,Card_State,Card_City,Card_Zip,FDT_CommercialServiceProvider,FDT_TransactionTime,FDT_TransactionID,FDT_ProessorReferenceNumber,FDT_ProcessorResponseMessage,FDT_ErrorMessage,FDT_OrderID,FDT_ApprovalCode,FDT_AVSResponse,FDT_TDate,FDT_TransactionResult,FDT_ProcessorApprovalCode,FDT_CalculatedTax,FDT_CalculatedShipping,FDT_TransactionScore,FDT_AuthenticationResponseCode,Created_On,Ip_Address) VALUES('".$Payment_Id."','".$User_ID."','".$orderAmount."','".mysql_real_escape_string($ResCC['Card_Type'])."','".mysql_real_escape_string($ResCC['Card_FirstName'])."',AES_ENCRYPT('".$ResCC['CC_NUMBER']."','".$EncryptKey."'),'".mysql_real_escape_string($ResCC['Card_Exp_Month'])."','".mysql_real_escape_string($ResCC['Card_Exp_Year'])."','".mysql_real_escape_string($ResCC['Card_Street'])."','".mysql_real_escape_string($ResCC['Card_State'])."','".mysql_real_escape_string($ResCC['Card_City'])."','".mysql_real_escape_string($ResCC['Card_Zip'])."','".$CommercialServiceProvider."','".$TransactionTime."','".$TransactionID."','".$ProessorReferenceNumber."','".$ProcessorResponseMessage."','".$ErrorMessage."','".$OrderID."','".$ApprovalCode."','".$AVSResponse."','".$TDate."','".$TransactionResult."','".$ProcessorApprovalCode."','".$CalculatedTax."','".$CalculatedShipping."','".$TransactionScore."','".$AuthenticationResponseCode."',Now(),'".$visit_ip."')";
		//mysql_query($sql);
