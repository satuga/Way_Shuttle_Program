...
$long=73;
	...
	$sql_long_col="merchant.geoLong";
		...
		if ($_REQUEST['dinedescription']!="" && $_REQUEST['dinedescription']!='Where are you going?') {
			$desc=explode(",",mysqli_real_escape_string($con,$_REQUEST['dinedescription']));
			if(count($desc)>1)
			{
				$Loc=explode(",",mysqli_real_escape_string($con,$_REQUEST['dinedescription']));
	  ...
	  // echo $Query1;exit;
		$res1=mysqli_query($con,$Query1) or die("line 154:-".mysqli_error($con));
	  ...
	  $Query=$Query1.$QueryJoin.$Query2.$QueryCommon;
		...
		$Totalpages 		= mysqli_query($con,"SELECT * FROM `tbl_control`") or die("line 324:-".mysqli_error($con));
		...
		if($TotalRecordCount<1)
		{
			...
			$result=mysqli_query($con,$SQL) or die("line 390:-".mysqli_error($con));
			$TotalRecordCount=mysqli_num_rows($result);
			$SQL=$SQL.$sql1;
			$Default=1;
