...
class Dine
{
	...
	{
		GLOBAL $con;
		$sql="SELECT id,Main_Menu,Start_Time,End_Time FROM menus WHERE menuStatus='Active' AND merchantID=".mysqli_real_escape_string($con,$_REQUEST['id'])." GROUP BY Main_Menu LIMIT 4";
	...
	function reservations()
	{
		...
		} else
		{
			...
			if($_REQUEST['Activity_ID']<>'')
			{
				...
				$sq1.= " AND orders.merchantID=".mysqli_real_escape_string($con,$_REQUEST['Activity_ID']);
			}
			else
			{
			...
			if($_REQUEST['F_Name']<>'') {
				...
				$sq1.= " AND tbl_registeration.firstname LIKE '%".mysqli_real_escape_string($con,$_REQUEST['F_Name'])."%' ";
			}
			if($_REQUEST['L_Name']<>'') {
				$sq.= " AND tbl_registeration.lastname LIKE '%".mysqli_real_escape_string($con,$_REQUEST['L_Name'])."%' ";
				$sq1.= " AND tbl_registeration.lastname LIKE '%".mysqli_real_escape_string($con,$_REQUEST['L_Name'])."%' ";
			}
			}
		}
		/*if($_REQUEST['Book_ID']<>'')
...
$message .=	"Content-Type: image/$ext; name=\"$google_icon_file.$ext\"\r\n"
	...
	function AddHelpfull()
	{
		...
		mysqli_query($con, $sql);
		$sql="INSERT INTO tbl_reviewhelpful(Help_Review,Help_Restaurant,Help_UserID,Help_Created)VALUES(".mysqli_real_escape_string($con,$_REQUEST['Review_ID']).",'".mysqli_real_escape_string($con,$_REQUEST['ID'])."','".$SESSION['User_ID']."',Now())";
		mysqli_query($con, $sql);
	}
	function AddLike()
	{
