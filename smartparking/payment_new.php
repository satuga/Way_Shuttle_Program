...
class PaymentService{
	...
	function __construct(){
		...
		$this->amount = isset($this->fields_val['amount']) ? $this->fields_val['amount'] :'';
                $this->is_postpaid = isset($this->fields_val['is_postpaid']) ? $this->fields_val['is_postpaid'] :'';
		...
		if ($this->user_id=="" || $this->txn_id=="" || $this->amount==""  || $this->card_id=="" || $this->pay_type=="" || $this->gate_id=="" || $this->is_postpaid=="")
		{
			//|| $this->stripe_token==""
		...
		if(strtolower($this->pay_type)=='creditcard' && $ValidPayment==1)
		{
			...
			try {
			    ...
			    if ($e instanceof Stripe_CardError || $e instanceof Stripe_InvalidRequestError
				  || $e instanceof Stripe_AuthenticationError || $e instanceof Stripe_ApiConnectionError
	...
	function fnAuthExit(){
		...
		$trans_update_result=mysqli_query($con,$sql_update_trans) or die($this->TransactionError());
                ...
                if(empty($this->is_postpaid)){
                    $sql_update_trans = "update tbl_paymenttransaction set to_date=now(), TotalAmount='TotalAmount+".$this->amount."' where TxnID='".$this->txn_id."'";
                    mysqli_query($con,$sql_update_trans) or die($this->TransactionError());
                } else {
                    $sql_update_trans = "update tbl_paymenttransaction set to_date=now(), TotalAmount='".$this->amount."' where TxnID='".$this->txn_id."'";
                    mysqli_query($con,$sql_update_trans) or die($this->TransactionError());
                }
