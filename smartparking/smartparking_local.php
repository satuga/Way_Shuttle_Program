...
class authService {
    ...
    function __construct() {
        ...
        if ($num == 1) {
            ...
            if ($this->gatemode != 1) {
                // user can exit from parking after 1 minutes
                $in_time = $rec['in_time'];
                $valid_datetime = strtotime(date('Y-m-d H:i:s', strtotime('+1 minutes', strtotime($in_time))));
                ...
                if ($num == 1 && $customerCanGoExit == 0) {
                    ...
                    if (!empty($payment_rec['is_postpaid'])) {
                        ...
                        //$this->UpdateParkingQntyWithOrderID($this->txn_id);
                        $this->updateCheckoutTime($this->txn_id);
                        ob_end_clean();
                        $retArr = array();
                        $retArr['status'] = 0;
                        $retArr['success'] = 0;
                        ...
                        if ($isExtraChargeForPost) {
                            ...
                            $P_ID = $this->garage_id;
                            $objPayinfoDO = $objParkMDA->getParkingPriceAndTotal($fromdate, $todate, $P_ID);
                            $extraTotalForPaid = $objPayinfoDO['total'];
                        } else {
                        ...
                        $retArr['current'] = $today;
                        $retArr['amount'] = $payment_rec['TotalAmount'] + $extraTotalForPaid; //->TotalAmount;
                        $retArr['creditcard'] = $card_array;
                        $retArr = removeNull($retArr);
                        echo json_encode($retArr);
                        exit;
            ...
            if ($this->gatemode != 2) {
                ...
                if ($num == 1) {
                    ...
                    $fromLessDate = date('Y-m-d H:i:s', strtotime($fromDate . ' -2 Hours'));
    ...
    function fnAuthExit() {
        ...
        $status = $flag > 0 ? 1 : 0;
        if ($status == 1 && $flag == 1) {
            //$this->increaseSpacesByPid($this->from_date, $this->to_date, $this->garage_id);
            ...
            if (!empty($parkingidForSpaces)) {
                if (strtotime($fromdateAvailability) < strtotime($todateAvailability)) {
                    while (strtotime($fromdateAvailability) < strtotime($todateAvailability)) {
                        $availability_upsql = "UPDATE tbl_parkingweekdayavailability set occupiedSpaces = (occupiedSpaces)-1 WHERE P_ID = $parkingidForSpaces AND P_fromDate = '" . $fromdateAvailability . "'";
...
//                $DBDATABASE = DB_DATABASE;
    ...
    function updateCheckoutTime($txtOrderID) {

        GLOBAL $con;
        $this->to_date = date('Y-m-d H:i:s');
        $sql_update_trans = "update tbl_paymenttransaction set to_date='" . $this->to_date . "' where TxnID='" . $txtOrderID . "'";
